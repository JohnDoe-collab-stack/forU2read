...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path], train_script_name: str) -> dict:
    ...
    snap_root = SNAP_DIR / f"aslmt_v8_pair_oodrequired_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v8_pair_oodrequired_snapshot",
        "ts": ts,
        "bundle_hash": bundle_hash,
        "bundle_hash_short": bundle_hash[:12],
        "snapshot_dir": str(snap_root),
        "train_script_name": str(train_src.name),
        "verify_script_name": "verify_aslmt_v8_double_barrier_hidden_target_pair_oodrequired.py",
...
def _run_one(*, manifest: dict, seed: int, device: str, steps: int, batch_size: int, lr: float, w_jepa: float, train_ood_ratio: float) -> dict:
    run_root = RUNS_DIR / f"aslmt_v8_pair_oodrequired_{manifest['ts']}_{manifest['bundle_hash_short']}"
    run_root.mkdir(parents=True, exist_ok=False)

    master_jsonl = run_root / f"v8_master_{manifest['ts']}_{manifest['bundle_hash_short']}.jsonl"
...
def _run_verify(*, snapshot_dir: Path, master_jsonl: Path, out_txt: Path) -> int:
    verify_script = snapshot_dir / "verify_aslmt_v8_double_barrier_hidden_target_pair_oodrequired.py"
...
def main() -> None:
    ...
    p.add_argument(
        "--train-script",
        type=str,
        default="aslmt_train_v8_double_barrier_hidden_target_seeded_pair_trainood.py",
    ...
    src_files = [
        HERE / "aslmt_env_v8_double_barrier_hidden_target.py",
        HERE / "aslmt_model_v8_double_barrier_jepa.py",
        HERE / "render_v8_paired_ctx.py",
        HERE / "verify_aslmt_v8_double_barrier_hidden_target_pair_oodrequired.py",
        HERE / train_script,
    ]
