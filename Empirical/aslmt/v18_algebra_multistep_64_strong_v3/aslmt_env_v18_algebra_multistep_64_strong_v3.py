...
def sample_episode(
    *,
    ...
    n_base = int(n) // 8
    if int(n_base) > int(obs_vocab):
        raise ValueError(
            "strong core requires n_base := n_states/8 <= obs_vocab "
            f"(got n_base={n_base}, obs_vocab={obs_vocab})"
        )
    ...
    for i in range(3, v):
        ...
        # Enforce strict variation inside every base bucket: a1 != a0 per bucket.
        a1 = _randint(g, 0, int(obs_vocab), (n_base,)).to(torch.long)
        if int(obs_vocab) <= 1:
            raise ValueError("obs_vocab must be >= 2 to build varying distractors")
        # Resample equal positions until all are different (terminates quickly for obs_vocab>=2).
        eq = a1.eq(a0)
        tries = 0
        while bool(eq.any().item()):
            tries += 1
            if tries > 32:
                raise RuntimeError("failed to sample distractors with a1 != a0 (too many resamples)")
            a1_new = _randint(g, 0, int(obs_vocab), (int(eq.sum().item()),)).to(torch.long)
            a1 = a1.clone()
            a1[eq] = a1_new
            eq = a1.eq(a0)
        if bool(ood):
    ...
    # Important: at t=0 both bit views are equally optimal (a symmetry / tie).
    # So the supervision target is set-valued at t=0: {bit1_idx, bit2_idx}.
    #
    # We still keep a canonical ordering for convenience/debugging, but training must NOT
    # treat opt_actions[0] as a unique label.
    ...
    # Sanity (exhaustive for v=O(8), cheap):
    # - closure must happen after 2 queries iff the chosen 2-view set contains both bit views
    # - no single view can close σ
    bit_set = {int(bit1_idx), int(bit2_idx)}
    if int(v) <= 16:
        # Single-view closure forbidden.
        for a in range(int(v)):
            if _is_closed_after_steps(tables=tables, sigma=sigma, x=x, actions=[int(a)]):
                raise RuntimeError(f"strong episode invariant violated: closed after 1 via a={a}")
        # Two-view closure equivalence.
        for a in range(int(v)):
            for b2 in range(int(v)):
                if a == b2:
                    continue
                closes = _is_closed_after_steps(tables=tables, sigma=sigma, x=x, actions=[int(a), int(b2)])
                should_close = ({int(a), int(b2)} == bit_set)
                if closes != should_close:
                    raise RuntimeError(
                        "strong episode invariant violated: "
                        f"closes={closes} but should_close={should_close} for actions={[a,b2]} "
                        f"bit_set={sorted(bit_set)}"
                    )
    else:
        # Fallback: check only the canonical bit plan closes in 2, and does not close in 1.
    ...
    return {
        ...
        "bit_idxs": torch.tensor(bit_idxs, dtype=torch.long),
