...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path]) -> dict:
    snap_root = SNAP_DIR / f"aslmt_v10_phaseA1_unified_nscalable_posloss_pairrank_kdet_{ts}_{bundle_hash[:12]}"
    snap_root.mkdir(parents=True, exist_ok=False)
    for p in src_files:
        (snap_root / p.name).write_text(p.read_text(encoding="utf-8"), encoding="utf-8")
    manifest = {
        "kind": "aslmt_v10_phaseA1_unified_nscalable_posloss_pairrank_kdet_snapshot",
...
def _run_train(
    *,
    ...
    train_script = snapshot_dir / "aslmt_train_v9_unified_double_barrier_minlift_seeded_pair_trainood_posloss_pairrank_nscalable_kdet.py"
...
def main() -> None:
    ...
    src_files = [
        v9_dir / "aslmt_model_v9_unified_double_barrier_minlift_kdet.py",
        v9_dir / "render_v9_unified_paired_ctx_nscalable.py",
        v9_dir / "aslmt_env_v9_unified_double_barrier_minlift_nscalable.py",
        v9_dir / "aslmt_train_v9_unified_double_barrier_minlift_seeded_pair_trainood_posloss_pairrank_nscalable_kdet.py",
    ...
    run_dir = RUNS_DIR / f"aslmt_v10_phaseA1_unified_nscalable_posloss_pairrank_kdet_{ts}_{bundle_hash[:12]}"
