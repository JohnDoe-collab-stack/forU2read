...
def _sample_ctxs_kind(*, n: int, seed: int, kind: str, img_size: int, n_classes: int) -> list[Ctx]:
    kind = str(kind)
    if kind not in ("iid", "ood", "ood2"):
        raise ValueError(f"unknown kind={kind!r} (expected iid|ood|ood2)")
    ctxs = _sample_ctxs(n=int(n), seed=int(seed), ood=bool(kind == "ood"), img_size=int(img_size), n_classes=int(n_classes))
    if kind != "ood2":
        return ctxs
    return [
        Ctx(
...
def _pair_eval_one(
    *,
    ...
    ctxs = _sample_ctxs_kind(n=int(cfg.n_ctx), seed=int(cfg.seed), kind=str(kind), img_size=int(cfg.img_size), n_classes=int(n_classes))
...
def main() -> None:
    ...
    p.add_argument("--train-ood2-ratio", type=float, default=0.0)
    ...
    cfg = TrainCfg(
        ...
        train_ood2_ratio=float(args.train_ood2_ratio),
    ...
    ds_iid = LawV3bUnifiedV2StrongQForcedDatasetNScalableSpaced2_64(
        size=4096, n_classes=int(n_classes), ood_kind="iid", img_size=int(cfg.img_size), seed=int(cfg.seed) + 1_000_000
    )
    ds_ood = LawV3bUnifiedV2StrongQForcedDatasetNScalableSpaced2_64(
        size=4096, n_classes=int(n_classes), ood_kind="ood", img_size=int(cfg.img_size), seed=int(cfg.seed) + 2_000_000
    )
    ds_ood2 = LawV3bUnifiedV2StrongQForcedDatasetNScalableSpaced2_64(
        size=4096, n_classes=int(n_classes), ood_kind="ood2", img_size=int(cfg.img_size), seed=int(cfg.seed) + 3_000_000
    )
    ...
    dl_ood2 = DataLoader(ds_ood2, batch_size=int(cfg.batch_size), shuffle=True, num_workers=0)
    it_iid = iter(dl_iid)
    it_ood = iter(dl_ood)
    it_ood2 = iter(dl_ood2)
    ...
    for step in range(1, int(cfg.steps) + 1):
        r = random.random()
        use_ood2 = r < float(cfg.train_ood2_ratio)
        use_ood = (not use_ood2) and (r < float(cfg.train_ood_ratio) + float(cfg.train_ood2_ratio))
        try:
            batch = next(it_ood2 if use_ood2 else (it_ood if use_ood else it_iid))
    ...
    def _eval_split(*, kind: str) -> dict:
        ...
        ds = LawV3bUnifiedV2StrongQForcedDatasetNScalableSpaced2_64(
            ...
            seed=int(cfg.seed) + (3_000_000 if kind == "ood2" else (2_000_000 if kind == "ood" else 0)),
        )
    ...
    metrics = {"iid": _eval_split(kind="iid"), "ood": _eval_split(kind="ood"), "ood2": _eval_split(kind="ood2")}

    pair_cfg = PairEvalCfg(n_ctx=int(cfg.pair_n_ctx), seed=int(cfg.seed), img_size=int(cfg.img_size))
    pair_eval = {
        "iid": _pair_eval_one(
            modelA=modelA, modelB_img=modelB_img, modelB_cue=modelB_cue, device=device, cfg=pair_cfg, kind="iid", n_classes=int(n_classes)
        ),
        "ood": _pair_eval_one(
            modelA=modelA, modelB_img=modelB_img, modelB_cue=modelB_cue, device=device, cfg=pair_cfg, kind="ood", n_classes=int(n_classes)
        ),
        "ood2": _pair_eval_one(
            modelA=modelA, modelB_img=modelB_img, modelB_cue=modelB_cue, device=device, cfg=pair_cfg, kind="ood2", n_classes=int(n_classes)
        ),
    }
    ...
    out = {
        ...
        "train_ood2_ratio": float(cfg.train_ood2_ratio),
    ...
    if str(args.out_faildump_jsonl).strip():
        ...
        if need_dump:
            ...
            fails_iid = _pair_eval_faildump_for_image_gate(
                modelA=modelA, device=device, cfg=pair_cfg, kind="iid", n_classes=int(n_classes), max_fails=int(cfg.faildump_max)
            )
            fails_ood = _pair_eval_faildump_for_image_gate(
                modelA=modelA, device=device, cfg=pair_cfg, kind="ood", n_classes=int(n_classes), max_fails=int(cfg.faildump_max)
            )
            fails_ood2 = _pair_eval_faildump_for_image_gate(
                modelA=modelA, device=device, cfg=pair_cfg, kind="ood2", n_classes=int(n_classes), max_fails=int(cfg.faildump_max)
            )
            rec = {
                "kind": "aslmt_law_v3b_unified_v2_strong_qforced_zread_faildump_image_gate_ood2",
                "seed": int(cfg.seed),
                "n_classes": int(n_classes),
                "z_classes": int(z_classes),
                "pair_n_ctx": int(cfg.pair_n_ctx),
                "img_size": int(cfg.img_size),
                "fails": fails_iid + fails_ood + fails_ood2,
            }
