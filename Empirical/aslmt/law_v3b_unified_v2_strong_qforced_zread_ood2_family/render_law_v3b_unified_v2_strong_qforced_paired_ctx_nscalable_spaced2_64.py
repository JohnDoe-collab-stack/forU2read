...
def render(ctx: Ctx, *, h: int, k: int, n_classes: int) -> dict[str, torch.Tensor]:
    ...
    inner_margin = 1 if not bool(ctx.ood) else 2
    ...
    if bool(ctx.ood2):
        g2 = torch.Generator()
        # Image noise must not depend on `h` or `k` (so the image barrier remains valid).
        g2.manual_seed(int(ctx.seed) + 424242)
        n_img_flips = int(torch.randint(low=10, high=31, size=(1,), generator=g2).item())
        ys2 = torch.randint(low=0, high=H, size=(n_img_flips,), generator=g2)
        xs2 = torch.randint(low=0, high=W, size=(n_img_flips,), generator=g2)
        image[ys2, xs2] = 1.0 - image[ys2, xs2]
