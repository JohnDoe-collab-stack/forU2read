...
def _partial_verify_ref_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_law_v3b_unified_v2_strong_qforced_matrix_ood2.py"
    verify_cmd = [
...
def main() -> None:
    ...
    sources = [
        ...
        (HERE / "verify_aslmt_law_v3b_unified_v2_strong_qforced_matrix_ood2.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_law_v3b_unified_v2_strong_qforced_zread_ood2_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_law_v3b_unified_v2_strong_qforced_zread_ood2_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"law_v3b_unified_v2_strong_qforced_zread_ood2_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
        ...
        for z in zs:
            for seed in seeds:
                ...
                cmd = [
                    ...
                    "--train-ood2-ratio",
                    str(float(args.train_ood2_ratio)),
                    "--pair-n-ctx",
                    str(int(pair_n_ctx)),
                    "--img-size",
                    str(int(img_size)),
