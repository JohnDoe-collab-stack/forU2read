...
def main() -> None:
    p = argparse.ArgumentParser(
        description="v20 algebra v3b proofpack v6: train → certify → verify (IID+OOD) + marginal no-go + strengthened minproof; trained baselines are checkpointed and verified."
    )
    ...
    run_dir = RUNS_DIR / f"aslmt_v20_algebra_v3b_proofpack_v6_{ts}_{bundle_short}"
    run_dir.mkdir(parents=True, exist_ok=False)
    snap_root = SNAP_DIR / f"aslmt_v20_algebra_v3b_proofpack_v6_{ts}_{bundle_short}"
