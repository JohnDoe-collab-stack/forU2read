...
from aslmt_env_v19_algebra_universal_v5_1 import V19AlgebraUniversalDataset
from aslmt_model_v19_algebra_universal_v5_1_actionz import (
...
def _eval_split(
    *,
    ...
    was_A = bool(modelA.training)
    was_Bv = bool(modelB_vis.training)
    was_Bc = bool(modelB_cue.training)
    modelA.eval()
    modelB_vis.eval()
    modelB_cue.eval()
    try:
        ...
        modelA.train(was_A)
        modelB_vis.train(was_Bv)
        modelB_cue.train(was_Bc)
    ...
    def _std(xs: list[float]) -> float:
        if len(xs) <= 1:
            return 0.0
        m = _mean(xs)
        v = float(sum((x - m) * (x - m) for x in xs) / float(len(xs) - 1))
        return float(math.sqrt(max(0.0, v)))

    out = {
        "A_acc": _mean(A_acc),
        "A_acc_std": _std(A_acc),
        "B_vis_acc": _mean(Bvis_acc),
        "B_vis_acc_std": _std(Bvis_acc),
        "B_cue_acc": _mean(Bcue_acc),
        "B_cue_acc_std": _std(Bcue_acc),
        "A_abl_acc": _mean(Abl_acc),
        "A_abl_acc_std": _std(Abl_acc),
        "A_swap_acc": _mean(Swap_acc),
        "A_swap_acc_std": _std(Swap_acc),
        "swap_action_follow_rate": _mean(swap_action_follow),
        "swap_action_follow_rate_std": _std(swap_action_follow),
    }
    for k in diag_keys:
        if diag_lists[k]:
            out[k] = _mean(diag_lists[k])
            out[f"{k}_std"] = _std(diag_lists[k])
    return out


def main() -> None:
    p = argparse.ArgumentParser(
        description="ASLMT v19 algebra universal v5.1 trainer (compatibility-aware belief + closed-only readout loss + stable eval logging)."
    )
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Optional: write modelA.state_dict() to this path at end of training.",
    )
    p.add_argument(
        "--log-eval-every",
        type=int,
        default=1000,
        help="Run a small stable eval (IID+OOD) every N steps and print the results.",
    )
    p.add_argument(
        "--log-eval-n",
        type=int,
        default=512,
        help="Number of eval episodes used for stable log-eval (per split).",
    )
    p.add_argument(
        "--log-eval-batch-size",
        type=int,
        default=64,
        help="Batch size for stable log-eval.",
    )
    args = p.parse_args()
    ...
    log_eval_every = int(args.log_eval_every)
    log_eval_n = int(args.log_eval_n)
    log_eval_bs = int(args.log_eval_batch_size)
    if profile == "smoke":
        if steps == 6000:
            steps = 300
        if batch_size == 64:
            batch_size = 32
        if log_eval_every == 1000:
            log_eval_every = 150
        if log_eval_n == 512:
            log_eval_n = 256
        if log_eval_bs == 64:
    ...
    save_modelA_state_dict = str(args.save_modelA_state_dict)
    ...
    for step in range(1, int(steps) + 1):
        ...
        should_print = (step % (250 if profile == "solid" else 50) == 0) or (step == steps)
        should_log_eval = (step % int(log_eval_every) == 0) or (step == steps)
        if should_print:
            with torch.no_grad():
            ...
            eval_suffix = ""
            if should_log_eval:
                eval_cfg_log = EvalCfg(n_eval=int(log_eval_n), batch_size=int(log_eval_bs))
                iid_log = _eval_split(
                    modelA=modelA,
                ...
                ood_log = _eval_split(
                    ...
                    max_steps=max_steps,
                    ood=True,
                    seed=int(args.seed) + 314159,
                    cfg=eval_cfg_log,
                )
                eval_suffix = (
                    f" | eval({int(log_eval_n)}) iid_A={float(iid_log['A_acc']):.4f} ood_A={float(ood_log['A_acc']):.4f}"
                    f" iid_A_std={float(iid_log.get('A_acc_std', 0.0)):.4f} ood_A_std={float(ood_log.get('A_acc_std', 0.0)):.4f}"
                    f" iid_closed={float(iid_log.get('closed_rate_roll', 0.0)):.3f} ood_closed={float(ood_log.get('closed_rate_roll', 0.0)):.3f}"
                    f" iid_closed_std={float(iid_log.get('closed_rate_roll_std', 0.0)):.3f} ood_closed_std={float(ood_log.get('closed_rate_roll_std', 0.0)):.3f}"
                )
            print(
    ...
    rec = {
        "kind": "aslmt_v19_algebra_universal_actionz_v5_1",
        "seed": int(args.seed),
        "n_states": int(n_states),
        "n_views": int(n_views),
        "y_classes": int(y_classes),
    ...
    if str(save_modelA_state_dict):
        out = Path(save_modelA_state_dict).expanduser()
        out.parent.mkdir(parents=True, exist_ok=True)
        torch.save(modelA.state_dict(), str(out))
        sha = hashlib.sha256(out.read_bytes()).hexdigest()
        print(f"SAVED modelA_state_dict={str(out)} sha256={sha}")
