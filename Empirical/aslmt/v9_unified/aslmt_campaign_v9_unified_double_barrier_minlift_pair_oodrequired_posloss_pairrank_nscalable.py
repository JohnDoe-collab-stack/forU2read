...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path], train_script_name: str) -> dict:
    snap_root = SNAP_DIR / f"aslmt_v9_unified_nscalable_posloss_pairrank_minlift_{ts}_{bundle_hash[:12]}"
    snap_root.mkdir(parents=True, exist_ok=False)
    for p in src_files:
        (snap_root / p.name).write_text(p.read_text(encoding="utf-8"), encoding="utf-8")
    manifest = {
        "kind": "aslmt_v9_unified_nscalable_posloss_pairrank_minlift_snapshot",
...
def main() -> None:
    ...
    src_files = [
        ...
        (Path(__file__).parent / "aslmt_env_v9_unified_double_barrier_minlift_nscalable.py").resolve(),
        (Path(__file__).parent / "render_v9_unified_paired_ctx_nscalable.py").resolve(),
        (Path(__file__).parent / "aslmt_model_v9_unified_double_barrier_minlift.py").resolve(),
        (Path(__file__).parent / "aslmt_train_v9_unified_double_barrier_minlift_seeded_pair_trainood_posloss_pairrank_nscalable.py").resolve(),
    ...
    run_dir = RUNS_DIR / f"aslmt_v9_unified_nscalable_posloss_pairrank_minlift_{ts}_{bundle_hash[:12]}"
    run_dir.mkdir(parents=True, exist_ok=False)
    master_jsonl = run_dir / f"v9_nscalable_posloss_pairrank_master_{ts}_{bundle_hash[:12]}.jsonl"
