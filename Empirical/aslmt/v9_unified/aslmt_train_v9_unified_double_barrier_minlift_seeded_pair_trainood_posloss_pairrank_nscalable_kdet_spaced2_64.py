...
def _sample_ctxs(*, n: int, seed: int, ood: bool, img_size: int, n_classes: int) -> list[Ctx]:
    ...
    # ensure occluder can fit in the image with a margin
    margin = 4
...
def main() -> None:
    ...
    ds_iid = V9UnifiedDoubleBarrierMinLiftDatasetNScalableSpaced2_64(size=50_000, n_classes=int(n_classes), ood=False, img_size=int(cfg.img_size), seed=int(cfg.seed))
    ds_ood = V9UnifiedDoubleBarrierMinLiftDatasetNScalableSpaced2_64(size=50_000, n_classes=int(n_classes), ood=True, img_size=int(cfg.img_size), seed=int(cfg.seed) + 1_000_000)
    ...
    for step in range(1, int(cfg.steps) + 1):
        ...
        lossA = lossA_seg + float(cfg.w_z) * lossA_z + float(cfg.w_pos) * lossA_pos + float(cfg.w_rank_img) * lossA_rank_img
    ...
    # Final eval (IID/OOD IoU + pair gates)
    def _eval_split(*, ood: bool) -> dict:
        ds = V9UnifiedDoubleBarrierMinLiftDatasetNScalableSpaced2_64(size=1024, n_classes=int(n_classes), ood=bool(ood), img_size=int(cfg.img_size), seed=int(cfg.seed) + (2_000_000 if ood else 0))
    ...
    out = {
        "kind": "aslmt_v9_unified_double_barrier_minlift_posloss_pairrank_spaced2_64",
        "seed": int(cfg.seed),
        "n_classes": int(n_classes),
