...
def _center_of_mass(mask01: torch.Tensor) -> torch.Tensor:
    ...
    returns: (B,2) centroid in [-1,1]^2 using the same xy grid as inputs.
    """
...
def _make_dataloaders(*, batch_size: int, steps: int, train_ood_ratio: float, n_classes: int, seed: int) -> tuple[DataLoader, DataLoader, DataLoader]:
    ...
    train_iid = V9UnifiedDoubleBarrierMinLiftDataset(size=n_iid, n_classes=n_classes, ood=False, seed=seed + 30_000)
    train_ood = V9UnifiedDoubleBarrierMinLiftDataset(size=n_ood, n_classes=n_classes, ood=True, seed=seed + 40_000)
...
def main() -> None:
    ...
    p.add_argument("--w-pos", type=float, default=0.0)
    ...
    for s in range(steps):
        ...
        lossA = lossA_seg + float(args.w_z) * lossA_z + float(args.w_k) * lossA_k
        if float(args.w_pos) != 0.0:
            ...
            lossA = lossA + float(args.w_pos) * lossA_pos
        else:
        ...
        if (s + 1) % int(args.log_every) == 0:
            print(
                ...
                f"(A_seg={float(lossA_seg.item()):.6f}, A_z={float(lossA_z.item()):.6f}, A_k={float(lossA_k.item()):.6f}, A_pos={float(lossA_pos.item()):.6f}, "
    ...
    payload = {
        "kind": "aslmt_v9_unified_double_barrier_minlift_posloss",
        "seed": int(args.seed),
        "n_classes": int(args.n_classes),
