...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path], train_script_name: str) -> dict:
    ...
    snap_root = SNAP_DIR / f"aslmt_v7_pair_iidonly_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v7_pair_iidonly_snapshot",
        "ts": ts,
...
def _run_one(*, manifest: dict, seed: int, device: str, steps: int, batch_size: int, lr: float, w_jepa: float) -> dict:
    run_root = RUNS_DIR / f"aslmt_v7_pair_iidonly_{manifest['ts']}_{manifest['bundle_hash_short']}"
    run_root.mkdir(parents=True, exist_ok=False)

    master_jsonl = run_root / f"v7_master_{manifest['ts']}_{manifest['bundle_hash_short']}.jsonl"
    train_script = Path(manifest["snapshot_dir"]) / "aslmt_train_v7_perfect_amodal_hidden_target_seeded_pair_iidonly.py"
...
def main() -> None:
    ...
    train_script = "aslmt_train_v7_perfect_amodal_hidden_target_seeded_pair_iidonly.py"
    src_files = [
