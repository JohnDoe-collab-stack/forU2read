...
This is identical to `aslmt_campaign_v7_perfect_amodal_hidden_target_pair_oodrequired.py` except for one hygiene fix:
the snapshot `manifest.files` list is derived from the files actually copied into the snapshot directory, so it cannot
accidentally list the train script twice when `--train-script` includes path components (e.g. `variants/pair/...`).

Per repo experiment rules, we do not edit the historical runner that has already produced cited artifacts.
"""
...
def main() -> None:
    ...
    p.add_argument(
        "--train-script",
        type=str,
        default="aslmt_train_v7_perfect_amodal_hidden_target_seeded_pair_trainood.py",
