...
def main() -> None:
    p = argparse.ArgumentParser(
        description="v20 algebra v3b proofpack v5: train → certify → verify (IID+OOD) + marginal no-go + minproof; tuned to harden image-gate (dice + stronger rank margin). Continues z-sweep even when verify fails for small z."
    )
    ...
    run_dir = RUNS_DIR / f"aslmt_v20_algebra_v3b_proofpack_v5_{ts}_{bundle_short}"
    run_dir.mkdir(parents=True, exist_ok=False)
    snap_root = SNAP_DIR / f"aslmt_v20_algebra_v3b_proofpack_v5_{ts}_{bundle_short}"
