...
def main() -> None:
    p = argparse.ArgumentParser(description="Postmortem dumper for v13 checkpoints: dump strict gate failures with margins.")
    ...
    if not isinstance(ckpt, dict) or "modelA_state_dict" not in ckpt:
        raise SystemExit("ckpt does not look like a v13 modelA checkpoint")
    ...
    for ood in (False, True):
        ...
        for i, ctx in enumerate(ctxs):
            ...
            if failures:
                _dump(
                    {
                        "kind": "aslmt_v13_postmortem_fail",
                        "ckpt": str(ckpt_path),
    ...
    print(json.dumps({"kind": "aslmt_v13_postmortem_summary", "total_checked": int(total_checked), "total_fail_records": int(total_fails), "out_jsonl": str(out_path)}))
