...
def _sample_ctxs(*, n: int, seed: int, ood: bool, img_size: int, n_classes: int, pos_stride: int) -> list:
    ...
    Ctx = sys.modules["render_v9_unified_paired_ctx_nscalable_spaced2_64"].Ctx
...
def main() -> None:
    p = argparse.ArgumentParser(description="Postmortem dumper for v14 checkpoints: dump strict gate failures with margins.")
    ...
    import render_v9_unified_paired_ctx_nscalable_spaced2_64 as render_mod
    from aslmt_model_v9_unified_double_barrier_minlift_kdet import V9MinLiftModelA

    ckpt_path = Path(str(args.ckpt)).expanduser().resolve()
    ckpt = torch.load(ckpt_path, map_location="cpu")
    if not isinstance(ckpt, dict) or "modelA_state_dict" not in ckpt:
        raise SystemExit("ckpt does not look like a v14 modelA checkpoint")
    ...
    for ood in (False, True):
        ...
        for i, ctx in enumerate(ctxs):
            ...
            if failures:
                _dump(
                    {
                        "kind": "aslmt_v14_postmortem_fail",
                        "ckpt": str(ckpt_path),
    ...
    print(json.dumps({"kind": "aslmt_v14_postmortem_summary", "total_checked": int(total_checked), "total_fail_records": int(total_fails), "out_jsonl": str(out_path)}))
