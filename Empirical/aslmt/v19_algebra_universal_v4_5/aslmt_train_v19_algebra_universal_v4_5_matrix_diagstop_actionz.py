...
from aslmt_env_v19_algebra_universal_v4_5 import V19AlgebraUniversalDataset
from aslmt_model_v19_algebra_universal_v4_5_actionz import (
    V19AlgebraCueOnlyBaseline,
    V19AlgebraUniversalModelA_ActionZ,
    V19AlgebraVisibleOnlyBaseline,
)
from aslmt_oracle_v19_algebra_universal_v4_5_common import (
    HorizonOracleCfg,
    candidate_mask,
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="ASLMT v19 algebra universal v4.5 trainer (learned encoder policy, horizon oracle supervision, action-as-z)."
    )
    ...
    for step in range(1, int(steps) + 1):
        ...
        # Candidate-consistency loss: concentrate the internal belief on the *exact* candidate set induced
        # by the rollout transcript (objective closure vs internalized closure).
        ...
        # Compute exact candidate masks on CPU (N is small; this keeps the definition identical to the oracle),
        # then move the batch mask once to device for differentiable belief-mass computation.
        tables_cpu = tables.detach().cpu()
        base_obs_cpu = base_obs.detach().cpu()
        ...
        loss = y_loss + 0.5 * y_loss_roll + q_loss + float(w_cand) * candidate_nll + 0.1 * (bvis_loss + bcue_loss)
    ...
    rec = {
        "kind": "aslmt_v19_algebra_universal_actionz_v4_5",
        "seed": int(args.seed),
        "n_states": int(n_states),
        "n_views": int(n_views),
        "y_classes": int(y_classes),
