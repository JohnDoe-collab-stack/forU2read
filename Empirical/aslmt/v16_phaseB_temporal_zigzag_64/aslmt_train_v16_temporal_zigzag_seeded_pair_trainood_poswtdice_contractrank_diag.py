...
from aslmt_env_v16_temporal_zigzag_nscalable_64 import V16TemporalZigzagDatasetNScalable64
from aslmt_model_v9_unified_double_barrier_minlift_kdet import V9CueOnlyBaseline, V9ImageOnlyBaseline, V9MinLiftModelA
from render_v16_temporal_zigzag_paired_ctx_64 import Ctx, POS_STRIDE, render, sample_ctx
...
def _sample_ctxs(*, n: int, seed: int, ood: bool, img_size: int, n_classes: int) -> list[Ctx]:
    return [
        sample_ctx(idx=int(i), n_classes=int(n_classes), ood=bool(ood), img_size=int(img_size), seed_base=int(seed))
...
def main() -> None:
    ...
    ds_iid = V16TemporalZigzagDatasetNScalable64(size=50_000, n_classes=int(n_classes), ood=False, img_size=int(cfg.img_size), seed=int(cfg.seed))
    ds_ood = V16TemporalZigzagDatasetNScalable64(
    ...
    def _eval_split(*, ood: bool) -> dict:
        ds = V16TemporalZigzagDatasetNScalable64(
    ...
    out = {
        "kind": "aslmt_v16_temporal_zigzag_poswtdice_contractrank_64",
        "seed": int(cfg.seed),
        "n_classes": int(n_classes),
    ...
    if str(args.out_faildump_jsonl).strip():
        ...
        if need_dump:
            ...
            rec = {
                        "kind": "aslmt_v16_temporal_zigzag_faildump_image_gate",
                "seed": int(cfg.seed),
                "n_classes": int(n_classes),
                "z_classes": int(z_classes),
