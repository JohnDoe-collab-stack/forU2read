...
def _partial_verify_ref_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_v15_phaseA2_matrix.py"
    verify_cmd = [
...
def main() -> None:
    ...
    sources = [
        ...
        (HERE / "aslmt_env_v15_unified_double_barrier_minlift_nscalable_spaced2_64.py"),
        (HERE / "render_v15_unified_paired_ctx_nscalable_spaced2_64.py"),
        (HERE / "aslmt_train_v15_unified_double_barrier_minlift_seeded_pair_trainood_poswtdice_contractrank_imgcuerank_nscalable_kdet_spaced2_64_diag.py"),
        (HERE / "verify_aslmt_v15_phaseA2_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)

    snap_root = SNAP_DIR / f"aslmt_v15_phaseA2_unified_nscalable_poswtdice_contractrank_imgcuerank_kdet_spaced2_64_{ts}_{bundle_hash[:12]}"
    snap_root.mkdir(parents=True, exist_ok=False)
    for pth in sources:
        shutil.copy2(pth, snap_root / pth.name)

    manifest = {
        "kind": "aslmt_v15_phaseA2_unified_nscalable_poswtdice_contractrank_imgcuerank_kdet_spaced2_64_snapshot",
        "timestamp": ts,
    ...
    run_dir = RUNS_DIR / f"aslmt_v15_phaseA2_unified_nscalable_poswtdice_contractrank_imgcuerank_kdet_spaced2_64_{ts}_{bundle_hash[:12]}"
    run_dir.mkdir(parents=True, exist_ok=False)
    master_jsonl = run_dir / f"v15_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n in n_list:
        ...
        for z in zs:
            for seed in seeds:
                train = snap_root / "aslmt_train_v15_unified_double_barrier_minlift_seeded_pair_trainood_poswtdice_contractrank_imgcuerank_nscalable_kdet_spaced2_64_diag.py"
