...
class LawV3bUnifiedV2StrongQForcedQueryModelA(nn.Module):
    """
    "zread" variant: query reads the *mediator* z (discrete), not z_logits (continuous).

    This matches the intended law_v3b spirit: action is a function of the internal mediator.
    """
    ...
    def __init__(self, *, z_classes: int, hid: int = 32):
        ...
        # Query head derived from z only (zread design).
        self.query_from_z = nn.Linear(self.z_classes, 2)

    def logits_query(self, cue_image: torch.Tensor) -> torch.Tensor:
        # Query is a readout from the discrete mediator z (one-hot).
        # We intentionally do not backprop query supervision into z_mlp here.
