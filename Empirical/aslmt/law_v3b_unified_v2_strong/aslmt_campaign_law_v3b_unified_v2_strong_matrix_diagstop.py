...
def _partial_verify_ref_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_law_v3b_unified_v2_strong_matrix.py"
    verify_cmd = [
...
def main() -> None:
    ...
    sources = [
        (HERE / "aslmt_model_law_v3b_unified_v2_strong_query.py"),
        (HERE / "aslmt_env_law_v3b_unified_v2_strong_nscalable_spaced2_64.py"),
        (HERE / "render_law_v3b_unified_v2_strong_paired_ctx_nscalable_spaced2_64.py"),
        (HERE / "aslmt_train_law_v3b_unified_v2_strong_seeded_pair_trainood_poswtdice_contractrank_diag.py"),
        (HERE / "verify_aslmt_law_v3b_unified_v2_strong_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_law_v3b_unified_v2_strong_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_law_v3b_unified_v2_strong_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"law_v3b_unified_v2_strong_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n in n_list:
        ...
        for z in zs:
            for seed in seeds:
                train = snap_root / "aslmt_train_law_v3b_unified_v2_strong_seeded_pair_trainood_poswtdice_contractrank_diag.py"
