...
def _sample_ctxs(*, n: int, seed: int, ood: bool, img_size: int, n_classes: int, pos_stride: int) -> list:
    ...
    from render_law_v3b_unified_v2_strong_paired_ctx_nscalable_spaced2_64 import Ctx
...
def main() -> None:
    p = argparse.ArgumentParser(description="Postmortem dumper for law_v3b_unified_v2_strong checkpoints: dump strict gate failures with margins.")
    ...
    import render_law_v3b_unified_v2_strong_paired_ctx_nscalable_spaced2_64 as render_mod
    from aslmt_model_law_v3b_unified_v2_strong_query import LawV3bUnifiedV2StrongQueryModelA
    ...
    if str(ckpt.get("kind", "")) != "aslmt_law_v3b_unified_v2_strong_poswtdice_contractrank_spaced2_64":
        raise SystemExit("ckpt kind mismatch (expected law_v3b_unified_v2_strong)")
    ...
    for ood in (False, True):
        ...
        for i, ctx in enumerate(ctxs):
            ...
            if failures:
                _dump(
                    {
                        "kind": "aslmt_law_v3b_unified_v2_strong_postmortem_fail",
                        "ckpt": str(ckpt_path),
    ...
    print(
        json.dumps(
            {
                "kind": "aslmt_law_v3b_unified_v2_strong_postmortem_summary",
