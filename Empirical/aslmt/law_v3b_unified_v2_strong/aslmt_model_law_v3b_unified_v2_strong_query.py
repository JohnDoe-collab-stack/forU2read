...
class LawV3bUnifiedV2StrongImageOnlyBaseline(nn.Module):
...
class LawV3bUnifiedV2StrongCueOnlyBaseline(nn.Module):
    def __init__(self, hid: int = 32):
        super().__init__()
        self.net = _UNet32(3, base=int(hid))
...
class LawV3bUnifiedV2StrongQueryModelA(nn.Module):
    """
    ...
    - The decoder is explicitly conditioned on both `z` and a query-provided `k` bit.

    This prevents the failure mode "the decision-time visible interface leaks k".
    """
    ...
    def __init__(self, *, z_classes: int, hid: int = 32):
        ...
        # Condition decoder on: (image, z one-hot, k one-hot from query response)
        self.dec = _UNet32(3 + self.z_classes + 2, base=int(hid))

        # v3b-style query head: choose which sensor/interface to query.
    ...
    def chosen_action(self, cue_image: torch.Tensor) -> torch.Tensor:
        ...
        Returns: (B,) long tensor with values 0 or 1 (two query actions).
        """
    ...
    @staticmethod
    def _k_from_res_bit(res_bit: torch.Tensor, *, dtype: torch.dtype) -> torch.Tensor:
        k = res_bit.to(dtype=torch.long)
        return F.one_hot(k, num_classes=2).to(dtype=dtype)
    ...
    def forward_with_res_bit(self, cue_image: torch.Tensor, image: torch.Tensor, *, res_bit: torch.Tensor) -> torch.Tensor:
        ...
        k_oh = self._k_from_res_bit(res_bit, dtype=image.dtype)
