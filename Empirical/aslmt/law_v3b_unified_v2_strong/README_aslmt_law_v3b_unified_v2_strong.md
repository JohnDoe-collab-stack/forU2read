# ASLMT law_v3b_unified_v2_strong
...
- Keep the v3b spine: internal state, query action, action-conditioned response (query bit), post-query state, decision.
...
- `render_law_v3b_unified_v2_strong_paired_ctx_nscalable_spaced2_64.py`
  - n-scalable spaced2_64 renderer with contract-preserving OOD cue corruption.
  - the decision-time image does not carry `k` (strong regime).
- `aslmt_env_law_v3b_unified_v2_strong_nscalable_spaced2_64.py`
  - dataset wrapper over the renderer.
- `aslmt_model_law_v3b_unified_v2_strong_query.py`
  - model A with a v3b-style query step:
    - query action is computed from cue-only internal state
    - response is an action-conditioned bit (`k` if the queried interface matches `h % 2`, else flipped)
    - decision is decoded from post-query state (image, z, response-bit)
  - baselines B image-only and cue-only.
- `aslmt_train_law_v3b_unified_v2_strong_seeded_pair_trainood_poswtdice_contractrank_diag.py`
  - training, pair-grade eval, ablation and swap gates, checkpoint saving, and faildump writing.
- `verify_aslmt_law_v3b_unified_v2_strong_matrix.py`
  - strict matrix verifier for the master JSONL.
- `aslmt_campaign_law_v3b_unified_v2_strong_matrix_diagstop.py`
  - snapshot plus hash campaign runner (diagstop on strict reference miss).
- `aslmt_postmortem_law_v3b_unified_v2_strong_from_ckpt_dump.py`
...
cd /mnt/c/Users/frederick/Documents/forU2read/Empirical/aslmt
  ...
  law_v3b_unified_v2_strong/aslmt_campaign_law_v3b_unified_v2_strong_matrix_diagstop.py \
  --profile solid \
  --device cuda \
...
python3 -u law_v3b_unified_v2_strong/aslmt_campaign_law_v3b_unified_v2_strong_matrix_diagstop.py \
  --profile smoke \
  --device cpu \
