...
from aslmt_env_law_v3b_unified_v2_strong_nscalable_spaced2_64 import LawV3bUnifiedV2StrongDatasetNScalableSpaced2_64
from aslmt_model_law_v3b_unified_v2_strong_query import (
    LawV3bUnifiedV2StrongCueOnlyBaseline,
    LawV3bUnifiedV2StrongImageOnlyBaseline,
    LawV3bUnifiedV2StrongQueryModelA,
)
from render_law_v3b_unified_v2_strong_paired_ctx_nscalable_spaced2_64 import Ctx, POS_STRIDE, render


def _env_res_bit(*, h: torch.Tensor, k: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
    """
    v3b-style action-conditioned response bit.

    Two actions, two "interfaces" (sensors). The correct interface is determined by `h % 2`.
    If we query the correct interface, the response equals `k`. Otherwise, it is flipped.
    """
    h2 = (h.to(torch.long) % 2).to(torch.long)
    a = action.to(torch.long)
    kk = k.to(torch.long)
    flip = (a != h2).to(torch.long)
    return (kk + flip) % 2
...
def _img_pair_contract_rank_loss(
    *,
    modelA: LawV3bUnifiedV2StrongQueryModelA,
    device: torch.device,
    n_classes: int,
    ctxs: list[Ctx],
    start_index: int,
    ...
    for j, ctx in enumerate(ctxs):
        ...
        a0 = modelA.chosen_action(cue0)
        a1 = modelA.chosen_action(cue1)
        r0 = _env_res_bit(h=x0["h"].unsqueeze(0).to(device), k=x0["k"].unsqueeze(0).to(device), action=a0)
        r1 = _env_res_bit(h=x1["h"].unsqueeze(0).to(device), k=x1["k"].unsqueeze(0).to(device), action=a1)
        p0 = modelA.forward_with_res_bit(cue0, image, res_bit=r0)
        p1 = modelA.forward_with_res_bit(cue1, image, res_bit=r1)
        ...
        res_pair = torch.cat([r0, r1], dim=0)
...
def _cue_pair_contract_rank_loss(
    *,
    ...
    for ctx in ctxs:
        ...
        a = modelA.chosen_action(cue)
        r0 = _env_res_bit(h=x0["h"].unsqueeze(0).to(device), k=x0["k"].unsqueeze(0).to(device), action=a)
        r1 = _env_res_bit(h=x1["h"].unsqueeze(0).to(device), k=x1["k"].unsqueeze(0).to(device), action=a)
        p0 = modelA.forward_with_res_bit(cue, img0, res_bit=r0)
        p1 = modelA.forward_with_res_bit(cue, img1, res_bit=r1)
...
def _pair_eval_one(
    *,
    modelA: LawV3bUnifiedV2StrongQueryModelA,
    modelB_img: LawV3bUnifiedV2StrongImageOnlyBaseline,
    modelB_cue: LawV3bUnifiedV2StrongCueOnlyBaseline,
    device: torch.device,
    cfg: PairEvalCfg,
    ood: bool,
    n_classes: int,
    ...
    for i, ctx in enumerate(ctxs):
        ...
        with torch.no_grad():
            a_im0 = modelA.chosen_action(cue_im0)
            a_im1 = modelA.chosen_action(cue_im1)
            r_im0 = _env_res_bit(h=x_im0["h"].unsqueeze(0).to(device), k=x_im0["k"].unsqueeze(0).to(device), action=a_im0)
            r_im1 = _env_res_bit(h=x_im1["h"].unsqueeze(0).to(device), k=x_im1["k"].unsqueeze(0).to(device), action=a_im1)
            pA_im0 = modelA.forward_with_res_bit(cue_im0, image_fixed, res_bit=r_im0)
            pA_im1 = modelA.forward_with_res_bit(cue_im1, image_fixed, res_bit=r_im1)

            a_cu = modelA.chosen_action(cue_fixed)
            r_cu0 = _env_res_bit(h=x_cu0["h"].unsqueeze(0).to(device), k=x_cu0["k"].unsqueeze(0).to(device), action=a_cu)
            r_cu1 = _env_res_bit(h=x_cu1["h"].unsqueeze(0).to(device), k=x_cu1["k"].unsqueeze(0).to(device), action=a_cu)
            pA_cu0 = modelA.forward_with_res_bit(cue_fixed, image_cu0, res_bit=r_cu0)
            pA_cu1 = modelA.forward_with_res_bit(cue_fixed, image_cu1, res_bit=r_cu1)

            pB_img = modelB_img(image_fixed)
            pB_cue = modelB_cue(cue_fixed)

            pA_im0_abl = modelA.ablated_forward_with_res_bit(cue_im0, image_fixed, res_bit=r_im0)
            pA_im1_abl = modelA.ablated_forward_with_res_bit(cue_im1, image_fixed, res_bit=r_im1)

            cue_pair = torch.cat([cue_im0, cue_im1], dim=0)
            img_pair = torch.cat([image_fixed, image_fixed], dim=0)
            res_pair = torch.cat([r_im0, r_im1], dim=0)
            perm = torch.tensor([1, 0], device=device, dtype=torch.long)
...
def _pair_eval_faildump_for_image_gate(
    *,
    ...
    for i, ctx in enumerate(ctxs):
        ...
        with torch.no_grad():
            a0 = modelA.chosen_action(cue_im0)
            a1 = modelA.chosen_action(cue_im1)
            r0 = _env_res_bit(h=x_im0["h"].unsqueeze(0).to(device), k=x_im0["k"].unsqueeze(0).to(device), action=a0)
            r1 = _env_res_bit(h=x_im1["h"].unsqueeze(0).to(device), k=x_im1["k"].unsqueeze(0).to(device), action=a1)
            pA_im0 = modelA.forward_with_res_bit(cue_im0, image_fixed, res_bit=r0)
            pA_im1 = modelA.forward_with_res_bit(cue_im1, image_fixed, res_bit=r1)
            cue_pair = torch.cat([cue_im0, cue_im1], dim=0)
            img_pair = torch.cat([image_fixed, image_fixed], dim=0)
            res_pair = torch.cat([r0, r1], dim=0)
            perm = torch.tensor([1, 0], device=device, dtype=torch.long)
...
def _bce_pos_weight_tensor(*, mode: str, tgt01: torch.Tensor) -> torch.Tensor | None:
    ...
    if mode == "batch":
        with torch.no_grad():
            pos = tgt01.sum()
            neg = (1.0 - tgt01).sum()
            w = (neg / pos.clamp_min(1.0)).clamp(min=1.0, max=100.0)
            return w.detach()
...
def main() -> None:
    ...
    p.add_argument("--faildump-max", type=int, default=200)
    args = p.parse_args()
    ...
    cfg = TrainCfg(
        ...
        faildump_max=int(args.faildump_max),
    )
    ...
    if float(cfg.w_rank_img) != 0.0 or float(cfg.w_rank_cue) != 0.0:
        if int(cfg.rank_n_ctx) <= 0 or int(cfg.rank_n_ctx) > int(cfg.pair_n_ctx):
            raise SystemExit("--rank-n-ctx must satisfy 1 <= rank_n_ctx <= pair_n_ctx for contract-aligned ranking")
        if int(cfg.pair_n_ctx) % int(cfg.rank_n_ctx) != 0:
            raise SystemExit("--pair-n-ctx must be a multiple of --rank-n-ctx for contract-aligned ranking")

    modelA = LawV3bUnifiedV2StrongQueryModelA(z_classes=int(z_classes)).to(device)
    modelB_img = LawV3bUnifiedV2StrongImageOnlyBaseline().to(device)
    modelB_cue = LawV3bUnifiedV2StrongCueOnlyBaseline().to(device)
    ...
    train_size = 5_000 if str(cfg.profile) == "smoke" else 50_000
    ds_iid = LawV3bUnifiedV2StrongDatasetNScalableSpaced2_64(
        size=int(train_size), n_classes=int(n_classes), ood=False, img_size=int(cfg.img_size), seed=int(cfg.seed)
    )
    ds_ood = LawV3bUnifiedV2StrongDatasetNScalableSpaced2_64(
        size=int(train_size), n_classes=int(n_classes), ood=True, img_size=int(cfg.img_size), seed=int(cfg.seed) + 1_000_000
    )
    ...
    for step in range(1, int(cfg.steps) + 1):
        ...
        lossA_seg_bce = F.binary_cross_entropy_with_logits(pA, tgt, pos_weight=posw) if posw is not None else F.binary_cross_entropy_with_logits(pA, tgt)
        ...
        lossA_q = F.cross_entropy(q_logits, (h % 2).to(torch.long))
        ...
        if do_print:
            with torch.no_grad():
                ...
                q_acc = float((action == (h % 2).to(torch.long)).float().mean().item())
            ...
            print(
                ...
                f"A_dice={float(lossA_seg_dice.item()):.6f}, A_z={float(lossA_z.item()):.6f}, "
                ...
                f"z_acc={z_acc:.4f} q_acc={q_acc:.4f} res_acc={res_acc:.4f}"
            )

    def _eval_split(*, ood: bool) -> dict:
        eval_size = 128 if str(cfg.profile) == "smoke" else 1024
        ds = LawV3bUnifiedV2StrongDatasetNScalableSpaced2_64(
        ...
        for b in dl:
            ...
            with torch.no_grad():
                ...
                q_accs.append(float((act == (h % 2).to(torch.long)).float().mean().item()))
    ...
    out = {
        "kind": "aslmt_law_v3b_unified_v2_strong_poswtdice_contractrank_spaced2_64",
        "seed": int(cfg.seed),
        "n_classes": int(n_classes),
    ...
    if str(args.out_faildump_jsonl).strip():
        ...
        if need_dump:
            ...
            rec = {
                "kind": "aslmt_law_v3b_unified_v2_strong_faildump_image_gate",
                "seed": int(cfg.seed),
                "n_classes": int(n_classes),
                "z_classes": int(z_classes),
