...
# Keep the same spaced2 semantics as the `spaced2` witness, but target larger `n`
# by using a larger default image size.
POS_STRIDE: int = 2
...
def render(ctx: Ctx, *, h: int, k: int, n_classes: int) -> dict[str, torch.Tensor]:
    ...
    Same unified witness structure as v9_unified:
      - `k` is visible in `image` as a present mark,
      - `h` is visible only in `cue_image` via a 2x2 marker,
      - the hidden target depends on both (`k` chooses orientation, `h` chooses position inside the occluder).

    Scaling constraint (checked):
      for both orientations, the number of discrete positions under spacing must satisfy:
        L >= POS_STRIDE*(n-1)+1
      where L = (inner_width)-4 (and similarly in y).
    """
    n_classes = int(n_classes)
    assert n_classes >= 2
    ...
    # Strong v3b-style regime:
    # the decision-time image carries geometry and occlusion but does not carry `k`.
    #
    # `k` is intended to be obtained only via a query response channel, not via the visible image.
    visible = scaffold.clamp(0.0, 1.0) * (1.0 - occlusion_mask)
    image = torch.clamp(visible + occlusion_mask, 0.0, 1.0)

    if ctx.ood:
        # OOD cue corruption (contract-preserving):
        #
        # The cue channel carries `h` through a 2x2 marker whose position encodes the class.
        # Therefore, the OOD corruption must not destroy this encoding, otherwise the task
        # becomes structurally impossible under a strict "1.0 everywhere" reference contract.
        #
        # We implement a deterministic, sparse flip process that:
        # - never flips inside the 2x2 marker (so the marker stays fully present),
        # - never places flips adjacent to each other (Chebyshev distance >= 2),
        #   which prevents accidentally creating a competing 2x2 block.
        g = torch.Generator()
        g.manual_seed(int(ctx.seed) + 1000 * int(hh))
        n_flips = int(torch.randint(low=10, high=21, size=(1,), generator=g).item())

        forbidden = set()
        # Forbid the marker itself.
        for yy in (my, my + 1):
            for xx in (mx, mx + 1):
        ...
        while len(flips) < n_flips and tries < max_tries:
            ...
            if (yy, xx) in forbidden:
                continue
            flips.append((yy, xx))
            # Forbid all adjacent sites (Chebyshev distance <= 1) to avoid 2x2 blocks.
            for dy in (-1, 0, 1):
                for dx in (-1, 0, 1):
                    y2 = yy + dy
                    x2 = xx + dx
...
def main() -> None:
    p = argparse.ArgumentParser(description="Render paired-ctx witnesses for ASLMT v9_unified (n-scalable, spaced2, 64x64).")
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (runs_dir / f"aslmt_v9_unified_nscalable_spaced2_64_paired_ctx_{suf}")
    ...
    checks = {
        "image_barrier": {"image_equal": bool(torch.equal(x_im0["image"], x_im1["image"])), "target_equal": bool(torch.equal(x_im0["hidden_target"], x_im1["hidden_target"]))},
        "cue_barrier": {"cue_equal": bool(torch.equal(x_cu0["cue_image"], x_cu1["cue_image"])), "target_equal": bool(torch.equal(x_cu0["hidden_target"], x_cu1["hidden_target"]))},
        "params": {"stride": int(POS_STRIDE), "hidden_thickness": int(HIDDEN_THICKNESS), "img_size": int(ctx.img_size)},
    }
    ...
    payload = {"ctx": asdict(ctx), "n_classes": n, "checks": checks, "image_pair": {"x": x_im0, "x_prime": x_im1, "k_fixed": k_fixed}, "cue_pair": {"x": x_cu0, "x_prime": x_cu1, "h_fixed": h_fixed}}
    torch.save(payload, tensors_path)

    manifest = {
        "kind": "aslmt_v9_unified_nscalable_spaced2_64_paired_ctx",
        "timestamp": ts,
