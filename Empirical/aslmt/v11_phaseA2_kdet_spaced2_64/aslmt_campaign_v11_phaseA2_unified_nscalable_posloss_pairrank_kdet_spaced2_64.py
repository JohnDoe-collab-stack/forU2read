...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path]) -> dict:
    snap_root = SNAP_DIR / f"aslmt_v11_phaseA2_unified_nscalable_posloss_pairrank_kdet_spaced2_64_{ts}_{bundle_hash[:12]}"
    snap_root.mkdir(parents=True, exist_ok=False)
    for p in src_files:
        (snap_root / p.name).write_text(p.read_text(encoding="utf-8"), encoding="utf-8")
    manifest = {
        "kind": "aslmt_v11_phaseA2_unified_nscalable_posloss_pairrank_kdet_spaced2_64_snapshot",
...
def _run_verify_matrix(*, snapshot_dir: Path, master_jsonl: Path, out_txt: Path, n_list: list[int], profile: str) -> int:
    verify = snapshot_dir / "verify_aslmt_v11_phaseA2_matrix.py"
    cmd = [
        sys.executable,
        "-u",
        str(verify),
...
def main() -> None:
    ...
    # Source files to snapshot (spaced2_64 witness + kdet model + v11 verify).
    v9 = ASLMT_DIR / "v9_unified"
    src_files = [
        v9 / "aslmt_model_v9_unified_double_barrier_minlift_kdet.py",
        v9 / "aslmt_train_v9_unified_double_barrier_minlift_seeded_pair_trainood_posloss_pairrank_nscalable_kdet_spaced2_64.py",
        ...
        (ASLMT_DIR / "v11_phaseA2_kdet_spaced2_64" / "verify_aslmt_v11_phaseA2_matrix.py"),
    ]
    ...
    run_dir = RUNS_DIR / f"aslmt_v11_phaseA2_unified_nscalable_posloss_pairrank_kdet_spaced2_64_{ts}_{bundle_hash[:12]}"
    run_dir.mkdir(parents=True, exist_ok=False)
    master_jsonl = run_dir / f"v11_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n in n_list:
        ...
        for z in zs:
            for seed in range(seed_from, seed_to + 1):
                ...
                train = snapshot_dir / "aslmt_train_v9_unified_double_barrier_minlift_seeded_pair_trainood_posloss_pairrank_nscalable_kdet_spaced2_64.py"
                cmd = [
                    sys.executable,
                    "-u",
                    str(train),
    ...
    if rc != 0:
        raise SystemExit(rc)
    print(f"[OK] v11 Phase A2 campaign complete. run_dir={run_dir}")
