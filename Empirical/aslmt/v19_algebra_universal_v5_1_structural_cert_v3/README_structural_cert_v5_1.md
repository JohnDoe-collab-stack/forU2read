...
- `verify_struct_aslmt_v19_algebra_universal_v5_1.py`:
  ...
  In particular it checks **response fidelity**: whenever `a_t != STOP`, the certificate must satisfy
  `responses[t] = tables[a_t, x]`.
  It also exports **all counterexamples** (not just counts):
  - `--out-report-json` includes a `violations_list` array with every violation record.
  - `--out-violations-jsonl` (optional) writes one JSON per violation for streaming/large runs.
...
- `--policy oracle`: sanity-check of the verifier + protocol invariants (does not demonstrate learning).
- `--policy model --model-state-dict ...`: structural demonstration attempt for a learned agent.
