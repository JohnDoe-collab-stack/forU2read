...
class DoubleBarrierHiddenTargetSequenceDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if k == 1:
            # Make `k` very salient under mean-pooling (unlike a tiny corner patch).
            # Place it on the top border so it is guaranteed to be outside the occluder
            # for all allowed (cx, cy, occ_half) ranges (occluder y0 >= 3).
            self._draw_rect(present_mark, 0, 0, W, 3, 1.0)
