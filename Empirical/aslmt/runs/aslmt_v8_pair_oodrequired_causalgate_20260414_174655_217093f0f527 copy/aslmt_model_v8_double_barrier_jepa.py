...
class V8DoubleBarrierCausalJEPA(nn.Module):
    """Stateful model A: reads cue first, then image; predicts hidden-only target from internal state."""
...
class V8ImageOnlyBaseline(nn.Module):
    """Baseline: image-only, no cue, no recurrent state."""
...
class V8CueOnlyBaseline(nn.Module):
    """Baseline: cue-only, no image, no recurrent state."""
