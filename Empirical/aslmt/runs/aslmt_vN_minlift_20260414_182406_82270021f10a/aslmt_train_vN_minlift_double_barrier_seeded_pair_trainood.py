...
from aslmt_env_vN_minlift_double_barrier import VNMinLiftDoubleBarrierDataset
from aslmt_model_vN_minlift_double_barrier import VNImageOnlyBaseline, VNCueOnlyBaseline, VNMinLiftModelA
...
def _pair_eval_one(
    *,
    modelA: VNMinLiftModelA,
    modelB_img: VNImageOnlyBaseline,
    modelB_cue: VNCueOnlyBaseline,
    device: torch.device,
    cfg: PairEvalCfg,
    ood: bool,
    n_classes: int,
...
def _make_dataloaders(*, batch_size: int, steps: int, train_ood_ratio: float, n_classes: int, seed: int) -> tuple[DataLoader, DataLoader, DataLoader]:
    ...
    iid_ds = VNMinLiftDoubleBarrierDataset(size=1024, n_classes=n_classes, ood=False, seed=seed + 10_000)
    ood_ds = VNMinLiftDoubleBarrierDataset(size=1024, n_classes=n_classes, ood=True, seed=seed + 20_000)

    train_parts = []
    if n_iid > 0:
        train_parts.append(VNMinLiftDoubleBarrierDataset(size=n_iid, n_classes=n_classes, ood=False, seed=seed + 30_000))
    if n_ood > 0:
        train_parts.append(VNMinLiftDoubleBarrierDataset(size=n_ood, n_classes=n_classes, ood=True, seed=seed + 40_000))
...
def train(args) -> dict:
    ...
    modelA = VNMinLiftModelA(z_classes=int(args.z_classes)).to(device)
    modelB_img = VNImageOnlyBaseline().to(device)
    modelB_cue = VNCueOnlyBaseline().to(device)
    ...
    for s in range(steps):
        ...
        # Govern the mediator: encourage z to carry the hidden class info from cue.
        # For z<n, this intentionally forces collisions (h mod z), which should
        # still fail the full pair-grade protocol.
    ...
    return {
        "kind": "aslmt_vN_minlift_double_barrier_pair_trainood",
        "steps": int(args.steps),
        "seed": int(args.seed),
        "n_classes": int(n_classes),
...
def main() -> None:
    ...
    print(
        f"Starting Training (vN minlift, n_classes={int(args.n_classes)}, z_classes={int(args.z_classes)}, "
