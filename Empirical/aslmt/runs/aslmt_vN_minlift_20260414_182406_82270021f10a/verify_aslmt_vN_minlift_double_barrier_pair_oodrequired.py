...
def main() -> None:
    ...
    print(
        f"[vN-REF] z={ref['z']} seeds={ref['seeds']} n_rows={ref['n_rows']} "
    ...
    # Smoke: only check structural barriers are true in reference group.
    # Solid: require full protocol closure + causal gates.
    ...
    if str(args.profile) == "solid":
        for z in sorted(by_z.keys()):
            ...
            print(f"[vN-<n] z={z} seeds={g['seeds']} iid_A_img_min={g['iid_A_img_min']:.4f} ood_A_img_min={g['ood_A_img_min']:.4f}")
    ...
    if errors:
        print("\n[FAIL] vN minlift checks did not pass:")
        for e in errors:
            print(" - " + str(e))
        raise SystemExit(1)

    print("\n[OK] vN minlift checks passed.")
