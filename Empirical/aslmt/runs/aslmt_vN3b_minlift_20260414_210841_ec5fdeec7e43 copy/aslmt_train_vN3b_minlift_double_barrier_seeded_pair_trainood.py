...
from aslmt_env_vN3b_minlift_double_barrier import VN3bMinLiftDoubleBarrierDataset
from aslmt_model_vN3b_minlift_double_barrier import VN3bCueOnlyBaseline, VN3bImageOnlyBaseline, VN3bMinLiftModelA
from render_vN3b_paired_ctx import Ctx, render
...
def _pair_eval_one(
    *,
    modelA: VN3bMinLiftModelA,
    modelB_img: VN3bImageOnlyBaseline,
    modelB_cue: VN3bCueOnlyBaseline,
    device: torch.device,
    cfg: PairEvalCfg,
    ood: bool,
    n_classes: int,
...
def _make_dataloaders(*, batch_size: int, steps: int, train_ood_ratio: float, n_classes: int, seed: int) -> tuple[DataLoader, DataLoader, DataLoader]:
    ...
    iid_ds = VN3bMinLiftDoubleBarrierDataset(size=1024, n_classes=n_classes, ood=False, seed=seed + 10_000)
    ood_ds = VN3bMinLiftDoubleBarrierDataset(size=1024, n_classes=n_classes, ood=True, seed=seed + 20_000)

    train_parts = []
    if n_iid > 0:
        train_parts.append(VN3bMinLiftDoubleBarrierDataset(size=n_iid, n_classes=n_classes, ood=False, seed=seed + 30_000))
    if n_ood > 0:
        train_parts.append(VN3bMinLiftDoubleBarrierDataset(size=n_ood, n_classes=n_classes, ood=True, seed=seed + 40_000))
...
def train(args) -> dict:
    ...
    modelA = VN3bMinLiftModelA(z_classes=int(args.z_classes)).to(device)
    modelB_img = VN3bImageOnlyBaseline().to(device)
    modelB_cue = VN3bCueOnlyBaseline().to(device)
    ...
    for s in range(steps):
        ...
        if s % max(1, steps // 12) == 0:
            print(f"[{s}] A_seg={lossA_seg.item():.4f} A_z={lossA_z.item():.4f} | B_img={lossB_img.item():.4f} | B_cue={lossB_cue.item():.4f}")
    ...
    with torch.no_grad():
        pair_iid = _pair_eval_one(modelA=modelA, modelB_img=modelB_img, modelB_cue=modelB_cue, device=device, cfg=pe_cfg, ood=False, n_classes=n_classes)
        pair_ood = _pair_eval_one(modelA=modelA, modelB_img=modelB_img, modelB_cue=modelB_cue, device=device, cfg=pe_cfg, ood=True, n_classes=n_classes)

    return {
        "kind": "aslmt_vN3b_minlift_double_barrier_pair_trainood",
        "steps": int(args.steps),
        "seed": int(args.seed),
        "n_classes": int(n_classes),
        "z_classes": int(args.z_classes),
        "cfg": {"batch_size": int(args.batch_size), "lr": float(args.lr), "train_ood_ratio": float(args.train_ood_ratio), "pair_n_ctx": int(args.pair_n_ctx), "w_z": float(args.w_z)},
        "iid": {"A_IoU": iid_A_iou, "B_img_IoU": iid_Bimg_iou, "B_cue_IoU": iid_Bcue_iou, "z_acc": iid_z_acc},
        "ood": {"A_IoU": ood_A_iou, "B_img_IoU": ood_Bimg_iou, "B_cue_IoU": ood_Bcue_iou, "z_acc": ood_z_acc},
...
def main() -> None:
    ...
    print(
        f"Starting Training (vN3b minlift, n_classes={int(args.n_classes)}, z_classes={int(args.z_classes)}, "
    ...
    print("\n--- EVALUATION (diagnostic IoU + z_acc) ---")
    print(f"IID -> A_IoU={row['iid']['A_IoU']:.4f} | z_acc={row['iid']['z_acc']:.4f} | B_img_IoU={row['iid']['B_img_IoU']:.4f} | B_cue_IoU={row['iid']['B_cue_IoU']:.4f}")
    print(f"OOD -> A_IoU={row['ood']['A_IoU']:.4f} | z_acc={row['ood']['z_acc']:.4f} | B_img_IoU={row['ood']['B_img_IoU']:.4f} | B_cue_IoU={row['ood']['B_cue_IoU']:.4f}")
