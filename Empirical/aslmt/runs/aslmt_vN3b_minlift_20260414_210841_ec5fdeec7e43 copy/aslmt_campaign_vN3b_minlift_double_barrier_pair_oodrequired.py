...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path], train_script_name: str) -> dict:
    ...
    manifest = {
        ...
        "verify_script_name": "verify_aslmt_vN3b_minlift_double_barrier_pair_oodrequired.py",
        "ts": ts,
    }
...
def _run_train(*, snapshot_dir: Path, out_jsonl: Path, out_txt: Path, seed: int, steps: int, device: str, n_classes: int, z_classes: int, profile: str) -> int:
    train_script = snapshot_dir / "aslmt_train_vN3b_minlift_double_barrier_seeded_pair_trainood.py"
...
def _run_verify(*, snapshot_dir: Path, master_jsonl: Path, out_txt: Path, n_classes: int, profile: str) -> int:
    verify_script = snapshot_dir / "verify_aslmt_vN3b_minlift_double_barrier_pair_oodrequired.py"
...
def main() -> None:
    ...
    src_files = [
        ...
        (Path(__file__).parent / "aslmt_env_vN3b_minlift_double_barrier.py").resolve(),
        (Path(__file__).parent / "render_vN3b_paired_ctx.py").resolve(),
        (Path(__file__).parent / "aslmt_model_vN3b_minlift_double_barrier.py").resolve(),
        (Path(__file__).parent / "aslmt_train_vN3b_minlift_double_barrier_seeded_pair_trainood.py").resolve(),
        (Path(__file__).parent / "verify_aslmt_vN3b_minlift_double_barrier_pair_oodrequired.py").resolve(),
    ]
    ...
    run_dir = RUNS_DIR / f"aslmt_vN3b_minlift_{ts}_{bundle_hash[:12]}"
    run_dir.mkdir(parents=True, exist_ok=False)
    master_jsonl = run_dir / f"vN3b_master_{ts}_{bundle_hash[:12]}.jsonl"

    for seed in seeds:
        for z_classes in z_list:
            log_txt = run_dir / f"train_{ts}_{bundle_hash[:12]}_n{n_classes}_z{z_classes}_seed{seed}.txt"
            rc = _run_train(snapshot_dir=Path(manifest["snapshot_dir"]), out_jsonl=master_jsonl, out_txt=log_txt, seed=int(seed), steps=int(steps), device=str(args.device), n_classes=int(n_classes), z_classes=int(z_classes), profile=profile)
            if rc != 0:
    ...
    if not master_jsonl.exists():
        verify_txt.write_text("SKIP VERIFY: master jsonl missing (all training runs failed before writing output).\n" f"master_jsonl={master_jsonl}\n", encoding="utf-8")
        verify_rc = 1
    else:
    ...
    payload = {"manifest": manifest, "run": {"run_dir": str(run_dir), "master_jsonl": str(master_jsonl), "verify_rc": int(verify_rc), "verify_txt": str(verify_txt)}}
