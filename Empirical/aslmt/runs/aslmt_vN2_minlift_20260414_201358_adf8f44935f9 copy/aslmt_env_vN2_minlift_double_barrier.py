...
class VN2MinLiftDoubleBarrierDataset(Dataset):
    """
    vN2 generalization of v8:

    - Hidden class `h ∈ {0..n-1}` is carried primarily by `cue_image`.
    - Present bit `k ∈ {0,1}` is carried only by `image`.
