...
def main() -> None:
    ...
    print(
        f"[vN2-REF] z={ref['z']} seeds={ref['seeds']} n_rows={ref['n_rows']} "
    ...
    if str(args.profile) == "solid":
        for z in sorted(by_z.keys()):
            ...
            print(f"[vN2-<n] z={z} seeds={g['seeds']} iid_A_img_min={g['iid_A_img_min']:.4f} ood_A_img_min={g['ood_A_img_min']:.4f}")
    ...
    if errors:
        print("\n[FAIL] vN2 minlift checks did not pass:")
        for e in errors:
            print(" - " + str(e))
        raise SystemExit(1)

    print("\n[OK] vN2 minlift checks passed.")
