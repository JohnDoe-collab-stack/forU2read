...
class VN2ImageOnlyBaseline(nn.Module):
...
class VN2CueOnlyBaseline(nn.Module):
    def __init__(self, hid: int = 32):
        super().__init__()
...
class _CueMarkerToXY(nn.Module):
    """
    Robustly extract a marker position from the cue image.

    Input is (cue, x, y). We predict an attention map, turn it into a probability
    distribution over pixels, then compute (x_mean, y_mean) via a weighted sum
    of the coordinate channels. This is stable under sparse flip noise.
    """
    ...
    def forward(self, cue: torch.Tensor) -> torch.Tensor:
        ...
        # confidence proxy: logsumexp of attention logits
        conf = torch.logsumexp(attn_flat, dim=-1, keepdim=True)
        return torch.cat([x_mean, y_mean, conf], dim=-1)


class _OccSummary(nn.Module):
    """Small global summary from (image,x,y) for occluder localization/size cues."""

    def __init__(self, in_ch: int = 3, hid: int = 32, out_dim: int = 16):
        super().__init__()
        h = int(hid)
        ...
        self.proj = nn.Linear(h, int(out_dim))
...
class VN2MinLiftModelA(nn.Module):
    """
    Model A with an explicit *discrete* mediator `z ∈ {0..z_classes-1}`.

    - z depends on (cue_image, image) to make the hidden class decode stable across ctx.
    - output is state-locked: depends only on (image, z), never on cue_image directly.
    """
    ...
    def __init__(self, *, z_classes: int, hid: int = 32):
        ...
        self.img_sum = _OccSummary(in_ch=3, hid=int(hid), out_dim=16)
    ...
    def z_logits(self, cue_image: torch.Tensor, image: torch.Tensor) -> torch.Tensor:
        marker = self.cue_to_xy(cue_image)  # (B,3)
        occ = self.img_sum(image)  # (B,16)
        return self.z_mlp(torch.cat([marker, occ], dim=-1))

    def _z_from_inputs(self, cue_image: torch.Tensor, image: torch.Tensor) -> torch.Tensor:
        logits = self.z_logits(cue_image, image)
    ...
    def forward(self, cue_image: torch.Tensor, image: torch.Tensor) -> torch.Tensor:
        z = self._z_from_inputs(cue_image, image)
