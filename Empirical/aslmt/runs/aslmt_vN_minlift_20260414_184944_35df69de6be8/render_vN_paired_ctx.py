...
def render(ctx: Ctx, *, h: int, k: int, n_classes: int) -> dict[str, torch.Tensor]:
    ...
    # Hidden class h is carried only by cue.
    ...
    # Encode `h` in cue as a small 2x2 marker at an h-dependent *2D* position
    # inside the occluder. This avoids collisions for small occluders (e.g. iid
    # occ_half=5) where a 1D-x encoding can have too few distinct positions.
    hh = int(h) % int(n_classes)
    max_x = max(1, (ix1 - ix0) - 1)  # number of valid top-left x positions for a 2x2 marker
    max_y = max(1, (iy1 - iy0) - 1)  # number of valid top-left y positions
    idx = int(hh)  # require injective mapping for the n we test (e.g. n<=25 in practice)
    marker_x = ix0 + int(idx % max_x)
    marker_y = iy0 + int((idx // max_x) % max_y)
    _draw_rect(cue, marker_x, marker_y, marker_x + 2, marker_y + 2, H, 1.0)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Render deterministic paired ctx witnesses for ASLMT vN.")
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (runs_dir / f"aslmt_vN_paired_ctx_{suf}")
