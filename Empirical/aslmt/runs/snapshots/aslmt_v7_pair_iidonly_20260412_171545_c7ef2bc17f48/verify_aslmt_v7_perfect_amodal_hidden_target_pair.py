...
def verify(master_jsonl: str) -> None:
    ...
    # Pair-based (COFRS-grade) checks: no IoU thresholds, only binary "both correct on a paired fiber witness".
    min_pair_iid_n = 10**9
    ...
    # IID paired witness must be present and perfect.
    if min_pair_iid_n == 10**9:
        errors.append("pair_eval.iid missing in master jsonl rows")
    else:
    ...
    # OOD paired witness is optional; if present, it must also be perfect.
    if min_pair_ood_n != 10**9:
    ...
    print(
        ...
        f"ood(n_ctx_min={'-' if min_pair_ood_n == 10**9 else min_pair_ood_n}, obs_barrier_ok={min_pair_ood_obs_barrier_ok}, "
        f"A_both_min={min_pair_ood_A_both:.4f}, B_both_max={max_pair_ood_B_both:.4f})"
    )

    if errors:
        print("\n[FAIL] V7 pair-grade checks did not pass:")
        for e in errors:
            print(f" - {e}")
        sys.exit(1)

    print("\n[OK] V7 pair-grade checks passed.")
    sys.exit(0)
