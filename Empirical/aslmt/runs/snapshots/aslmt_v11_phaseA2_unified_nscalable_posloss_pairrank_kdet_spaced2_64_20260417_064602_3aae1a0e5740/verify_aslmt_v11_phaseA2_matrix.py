...
def main() -> None:
    p = argparse.ArgumentParser(description="Strict verifier for ASLMT v11 Phase A2 matrix runs.")
    ...
    # Collect failures in a human-readable way.
    failures: list[str] = []

    # For each n, check all required z and seed coverage.
    for n in n_list:
        if str(args.z_policy) == "explicit":
            zs = list(z_list)
        else:
        ...
        for z in zs:
            # Gather seeds present for this (n,z).
            ...
            # Reference group: z=n must pass all strict gates.
            if int(z) == int(n):
                for sp in ("iid", "ood"):
                ...
                # Under-capacity groups: must fail to close the image barrier (at least once).
                for sp in ("iid", "ood"):
                    if not _all_barriers_ok(sp):
    ...
    if failures:
        print("\n[FAIL] v11 Phase A2 matrix checks did not pass:")
        for s in failures:
            print("-", s)
        raise SystemExit(1)

    print("\n[OK] v11 Phase A2 matrix checks passed.")
