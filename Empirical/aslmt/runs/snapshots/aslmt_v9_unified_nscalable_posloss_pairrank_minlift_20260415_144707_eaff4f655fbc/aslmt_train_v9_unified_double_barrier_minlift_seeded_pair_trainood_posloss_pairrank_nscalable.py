...
def _sample_ctxs(*, n: int, seed: int, ood: bool, img_size: int) -> list[Ctx]:
    raise RuntimeError("old _sample_ctxs signature should not be used (missing n_classes)")
...
def _sample_ctxs_for_n(*, n: int, seed: int, ood: bool, img_size: int, n_classes: int) -> list[Ctx]:
    g = torch.Generator()
...
def _img_pair_rank_loss(
    *,
    ...
    """
    ctxs = _sample_ctxs_for_n(n=int(n_ctx), seed=int(seed), ood=bool(ood), img_size=32, n_classes=int(n_classes))
    g = torch.Generator()
...
def _pair_eval_one(
    *,
    ...
    ctxs = _sample_ctxs_for_n(n=int(cfg.n_ctx), seed=int(cfg.seed), ood=bool(ood), img_size=int(cfg.img_size), n_classes=int(n_classes))
