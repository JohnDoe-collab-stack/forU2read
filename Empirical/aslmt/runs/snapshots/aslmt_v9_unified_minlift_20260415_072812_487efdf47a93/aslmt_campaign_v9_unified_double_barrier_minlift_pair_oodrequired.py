...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path], train_script_name: str) -> dict:
    ...
    manifest = {
        ...
        "verify_script_name": "verify_aslmt_v9_unified_double_barrier_minlift_pair_oodrequired.py",
        "ts": ts,
    }
...
def _run_train(*, snapshot_dir: Path, out_jsonl: Path, out_txt: Path, seed: int, steps: int, device: str, n_classes: int, z_classes: int, profile: str) -> int:
    train_script = snapshot_dir / "aslmt_train_v9_unified_double_barrier_minlift_seeded_pair_trainood.py"
...
def _run_verify(*, snapshot_dir: Path, master_jsonl: Path, out_txt: Path, n_classes: int, profile: str) -> int:
    verify_script = snapshot_dir / "verify_aslmt_v9_unified_double_barrier_minlift_pair_oodrequired.py"
...
def main() -> None:
    ...
    src_files = [
        ...
        (Path(__file__).parent / "aslmt_env_v9_unified_double_barrier_minlift.py").resolve(),
        (Path(__file__).parent / "render_v9_unified_paired_ctx.py").resolve(),
        (Path(__file__).parent / "aslmt_model_v9_unified_double_barrier_minlift.py").resolve(),
        (Path(__file__).parent / "aslmt_train_v9_unified_double_barrier_minlift_seeded_pair_trainood.py").resolve(),
        (Path(__file__).parent / "verify_aslmt_v9_unified_double_barrier_minlift_pair_oodrequired.py").resolve(),
    ]
    ...
    run_dir = RUNS_DIR / f"aslmt_v9_unified_minlift_{ts}_{bundle_hash[:12]}"
    run_dir.mkdir(parents=True, exist_ok=False)
    master_jsonl = run_dir / f"v9_master_{ts}_{bundle_hash[:12]}.jsonl"
