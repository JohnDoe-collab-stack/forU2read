...
def train(args) -> dict:
    ...
    for s in range(steps):
        ...
        # encourage z to be confident (sharp) to make capacity pressure visible
        z_logits = modelA.z_logits(cue)
        z_prob = F.softmax(z_logits, dim=-1)
        z_ent = -(z_prob * torch.log(z_prob + 1e-9)).sum(dim=-1).mean()

        loss = lossA + lossB_img + lossB_cue + float(args.w_z) * z_ent
        ...
        if (s + 1) % int(args.log_every) == 0:
            print(f"step={s+1}/{steps} loss={float(loss.item()):.6f} (A={float(lossA.item()):.6f}, Bimg={float(lossB_img.item()):.6f}, Bcue={float(lossB_cue.item()):.6f}, z_ent={float(z_ent.item()):.6f})")
