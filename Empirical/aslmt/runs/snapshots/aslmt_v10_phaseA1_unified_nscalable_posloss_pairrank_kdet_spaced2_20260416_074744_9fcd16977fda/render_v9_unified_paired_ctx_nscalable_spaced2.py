...
def render(ctx: Ctx, *, h: int, k: int, n_classes: int) -> dict[str, torch.Tensor]:
    """
    n-scalable + spaced variant:

    - keeps the same unified witness structure,
    - but makes the hidden-class position map spaced (stride=2) to reduce overlap between
      competing targets on the image-barrier.

    Preconditions (must be ensured by sampling):
      - both orientations have enough discrete positions for stride=2:
          L >= POS_STRIDE*(n-1)+1
        where L = (ix1-ix0)-4 (or analog in y).
    """
    n_classes = int(n_classes)
    assert n_classes >= 2
    ...
    # Number of distinct unit positions available (per orientation).
    Lx = int((ix1 - ix0) - 4)
    Ly = int((iy1 - iy0) - 4)
    ...
    if Lx < needed or Ly < needed:
        raise ValueError(
            ...
            f"needed={needed}, Lx={Lx}, Ly={Ly}, occ_half={occ_half}, ood={ctx.ood}"
        )
...
def main() -> None:
    p = argparse.ArgumentParser(description="Render paired-ctx witnesses for ASLMT v9_unified (n-scalable, spaced2).")
    ...
    p.add_argument("--occ-half", type=int, default=7)
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (runs_dir / f"aslmt_v9_unified_nscalable_spaced2_paired_ctx_{suf}")
    ...
    checks = {
        ...
        "params": {"stride": int(POS_STRIDE), "hidden_thickness": int(HIDDEN_THICKNESS)},
    }
    ...
    manifest = {
        "kind": "aslmt_v9_unified_nscalable_spaced2_paired_ctx",
        "timestamp": ts,
