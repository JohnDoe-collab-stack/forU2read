...
def _cue_marker_pos(*, h: int, n_classes: int, img_size: int) -> tuple[int, int]:
    """
    Encode h ∈ Fin n as a 2x2 marker position in cue_image.
    """
...
def render(ctx: Ctx, *, h: int, k: int, n_classes: int) -> dict[str, torch.Tensor]:
    ...
    Decision-time observable:
      - image carries k (present bit) and visible scaffold/occluder
      - cue_image carries h (hidden class) via a marker

    Target:
      - depends on both (h,k), so image-only and cue-only are each insufficient
      - chosen so that, for fixed k, varying h yields n distinct targets (min-lift pressure)
    """
    n_classes = int(n_classes)
    assert n_classes >= 2
    ...
    # Present bit k, carried only by the decision-time image (outside occluder).
    present_mark = torch.zeros_like(scaffold)
    if int(k) == 1:
        _draw_rect(present_mark, 0, 0, W, 3, H, 1.0)

    # Hidden class h, carried only by cue_image via a 2x2 marker.
    ...
    # Hidden-only target inside occluder depends on (h,k).
    # - k chooses orientation (forces cue-only barrier)
    # - h chooses position (forces min-lift)
    ...
    # OOD nuisance: deterministic sparse distractor flips in cue only.
    if ctx.ood:
        g = torch.Generator()
...
def main() -> None:
    p = argparse.ArgumentParser(description="Render paired-ctx witnesses for ASLMT v9_unified.")
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (runs_dir / f"aslmt_v9_unified_paired_ctx_{suf}")
    ...
    manifest = {
        "kind": "aslmt_v9_unified_paired_ctx",
        "timestamp": ts,
