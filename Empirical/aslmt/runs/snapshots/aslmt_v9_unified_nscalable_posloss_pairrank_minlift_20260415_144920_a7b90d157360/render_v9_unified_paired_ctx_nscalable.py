...
def render(ctx: Ctx, *, h: int, k: int, n_classes: int) -> dict[str, torch.Tensor]:
    """
    n-scalable variant: ensure the image-barrier is valid for arbitrary n by
    making the (fixed-k) position map h ↦ position injective within the inner region.

    Preconditions (must be ensured by sampling):
      - both orientations have enough discrete positions: Lx>=n and Ly>=n.
    """
    n_classes = int(n_classes)
    assert n_classes >= 2
    ...
    # Number of distinct positions available (per orientation).
    Lx = int((ix1 - ix0) - 4)
    Ly = int((iy1 - iy0) - 4)
    if Lx < n_classes or Ly < n_classes:
        raise ValueError(f"ctx does not support n={n_classes}: Lx={Lx}, Ly={Ly}, occ_half={occ_half}, ood={ctx.ood}")
...
def main() -> None:
    p = argparse.ArgumentParser(description="Render paired-ctx witnesses for ASLMT v9_unified (n-scalable).")
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (runs_dir / f"aslmt_v9_unified_nscalable_paired_ctx_{suf}")
    ...
    manifest = {
        "kind": "aslmt_v9_unified_nscalable_paired_ctx",
        "timestamp": ts,
