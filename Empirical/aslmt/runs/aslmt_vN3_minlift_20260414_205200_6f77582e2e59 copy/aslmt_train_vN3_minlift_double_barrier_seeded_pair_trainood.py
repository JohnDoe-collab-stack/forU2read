...
from aslmt_env_vN3_minlift_double_barrier import VN3MinLiftDoubleBarrierDataset
from aslmt_model_vN3_minlift_double_barrier import VN3CueOnlyBaseline, VN3ImageOnlyBaseline, VN3MinLiftModelA
from render_vN3_paired_ctx import Ctx, render
...
def _pair_eval_one(
    *,
    modelA: VN3MinLiftModelA,
    modelB_img: VN3ImageOnlyBaseline,
    modelB_cue: VN3CueOnlyBaseline,
    device: torch.device,
    cfg: PairEvalCfg,
    ood: bool,
    n_classes: int,
...
def _make_dataloaders(*, batch_size: int, steps: int, train_ood_ratio: float, n_classes: int, seed: int) -> tuple[DataLoader, DataLoader, DataLoader]:
    ...
    iid_ds = VN3MinLiftDoubleBarrierDataset(size=1024, n_classes=n_classes, ood=False, seed=seed + 10_000)
    ood_ds = VN3MinLiftDoubleBarrierDataset(size=1024, n_classes=n_classes, ood=True, seed=seed + 20_000)

    train_parts = []
    if n_iid > 0:
        train_parts.append(VN3MinLiftDoubleBarrierDataset(size=n_iid, n_classes=n_classes, ood=False, seed=seed + 30_000))
    if n_ood > 0:
        train_parts.append(VN3MinLiftDoubleBarrierDataset(size=n_ood, n_classes=n_classes, ood=True, seed=seed + 40_000))
...
def train(args) -> dict:
    ...
    modelA = VN3MinLiftModelA(z_classes=int(args.z_classes)).to(device)
    modelB_img = VN3ImageOnlyBaseline().to(device)
    modelB_cue = VN3CueOnlyBaseline().to(device)
    ...
    return {
        "kind": "aslmt_vN3_minlift_double_barrier_pair_trainood",
        "steps": int(args.steps),
        "seed": int(args.seed),
        "n_classes": int(n_classes),
...
def main() -> None:
    ...
    print(
        f"Starting Training (vN3 minlift, n_classes={int(args.n_classes)}, z_classes={int(args.z_classes)}, "
