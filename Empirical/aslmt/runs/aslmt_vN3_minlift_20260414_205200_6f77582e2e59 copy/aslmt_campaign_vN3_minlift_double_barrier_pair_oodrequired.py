...
def _make_snapshot(*, ts: str, bundle_hash: str, src_files: list[Path], train_script_name: str) -> dict:
    ...
    manifest = {
        ...
        "verify_script_name": "verify_aslmt_vN3_minlift_double_barrier_pair_oodrequired.py",
        "ts": ts,
    }
...
def _run_train(
    *,
    ...
    train_script = snapshot_dir / "aslmt_train_vN3_minlift_double_barrier_seeded_pair_trainood.py"
...
def _run_verify(*, snapshot_dir: Path, master_jsonl: Path, out_txt: Path, n_classes: int, profile: str) -> int:
    verify_script = snapshot_dir / "verify_aslmt_vN3_minlift_double_barrier_pair_oodrequired.py"
...
def main() -> None:
    ...
    src_files = [
        ...
        (Path(__file__).parent / "aslmt_env_vN3_minlift_double_barrier.py").resolve(),
        (Path(__file__).parent / "render_vN3_paired_ctx.py").resolve(),
        (Path(__file__).parent / "aslmt_model_vN3_minlift_double_barrier.py").resolve(),
        (Path(__file__).parent / "aslmt_train_vN3_minlift_double_barrier_seeded_pair_trainood.py").resolve(),
        (Path(__file__).parent / "verify_aslmt_vN3_minlift_double_barrier_pair_oodrequired.py").resolve(),
    ]
    ...
    run_dir = RUNS_DIR / f"aslmt_vN3_minlift_{ts}_{bundle_hash[:12]}"
    run_dir.mkdir(parents=True, exist_ok=False)
    master_jsonl = run_dir / f"vN3_master_{ts}_{bundle_hash[:12]}.jsonl"
