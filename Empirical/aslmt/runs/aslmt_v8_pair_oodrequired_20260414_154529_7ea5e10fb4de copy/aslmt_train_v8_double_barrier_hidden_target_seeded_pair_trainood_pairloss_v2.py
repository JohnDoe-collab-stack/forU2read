...
def _pair_rank_loss(pred_logits: torch.Tensor, target_right: torch.Tensor, target_wrong: torch.Tensor) -> torch.Tensor:
    # This matches the verifier's decision rule (overlap comparison), but turns it into a smooth loss.
...
def train(args) -> None:
    ...
    print(
        f"Starting Training (V8 Double-Barrier, pair-loss v2, train_ood_ratio={train_ood_ratio:.2f}, "
        f"steps={steps}, seed={args.seed}, device={args.device})..."
    )

    for step in range(steps):
        # Sample half IID, half OOD according to train_ood_ratio.
        ...
        # Split contexts: train both barriers each step without quadrupling batch size.
        mid = len(ctxs) // 2
        ctxs_im = ctxs[:mid]
        ctxs_cu = ctxs[mid:]
        ...
        loss_seg_im = F.binary_cross_entropy_with_logits(pA_im, target_im)
        loss_seg_cu = F.binary_cross_entropy_with_logits(pA_cu, target_cu)
        loss_seg = 0.5 * (loss_seg_im + loss_seg_cu)

        # Pair-grade ranking loss aligned with verifier.
        ...
        # JEPA on both halves (use hidden_target as x_full, consistent with v7/v8).
        ...
        # Baseline training (diagnostic; not needed for the barrier to hold, but keeps B strong).
        modelB_img.train()
        modelB_cue.train()
        ...
        loss_B_img = F.binary_cross_entropy_with_logits(pB_img, targets_all)
        optB_img.zero_grad()
        loss_B_img.backward()
        optB_img.step()

        pB_cue = modelB_cue(cues_all)
        loss_B_cue = F.binary_cross_entropy_with_logits(pB_cue, targets_all)
        optB_cue.zero_grad()
        loss_B_cue.backward()
        optB_cue.step()
        ...
        if step % 250 == 0:
            print(
                f"[{step}] A_seg={loss_seg.item():.4f} A_pair={loss_pair.item():.4f} (im {loss_pair_im.item():.4f} cu {loss_pair_cu.item():.4f}) "
                f"JEPA={loss_jepa.item():.4f} | B_img={loss_B_img.item():.4f} B_cue={loss_B_cue.item():.4f}"
            )
    ...
    def eval_iou(*, ood: bool) -> tuple[float, float, float]:
        ...
        for ctx in ctxs_eval:
            # Use all 4 combinations to avoid accidentally evaluating a degenerate subfamily.
            for h in (0, 1):
                for k in (0, 1):
    ...
    if args.out_jsonl:
        row = {
            "kind": "aslmt_v8_double_barrier_hidden_target_pair_trainood_pairloss_v2",
            "steps": int(args.steps),
            "seed": int(args.seed),
            "cfg": {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch-size", type=int, default=128, help="Number of ctx per step; internal batch is ~2x.")
