...
def _partial_verify_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_v19_algebra_universal_v5_matrix.py"
    verify_cmd = [
        str(py),
        "-u",
        str(verify),
...
def main() -> None:
    ...
    sources = [
        ...
        (HERE / "aslmt_oracle_v19_algebra_universal_v5_common.py"),
        (HERE / "aslmt_env_v19_algebra_universal_v5.py"),
        (HERE / "aslmt_model_v19_algebra_universal_v5_actionz.py"),
        (HERE / "aslmt_train_v19_algebra_universal_v5_matrix_diagstop_actionz.py"),
        (HERE / "verify_aslmt_v19_algebra_universal_v5_matrix.py"),
        # Protocol provenance (document + orchestrator + independent audit).
        (HERE / "aslmt_campaign_v19_algebra_universal_v5_matrix_diagstop_actionz.py"),
        (HERE / "audit_v19_algebra_universal_v5_algebra.py"),
        (HERE / "README_aslmt_v19_algebra_universal_v5.md"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_v19_algebra_universal_actionz_v5_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v19_algebra_universal_actionz_v5_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"v19_algebra_universal_actionz_v5_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n_states in n_list:
        for seed in seeds:
            train = snap_root / "aslmt_train_v19_algebra_universal_v5_matrix_diagstop_actionz.py"
