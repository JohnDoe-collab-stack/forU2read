...
from aslmt_model_v18_algebra_multistep_64_actionz import V18AlgebraMultistepModelA_ActionZ
...
def _eval_split(
    *,
    modelA: V18AlgebraMultistepModelA_ActionZ,
...
def main() -> None:
    p = argparse.ArgumentParser(description="ASLMT v18 algebra multistep matrix trainer (action-as-z diagstop).")
    ...
    modelA = V18AlgebraMultistepModelA_ActionZ(cfgA).to(device)
    ...
    rec = {
        "kind": "aslmt_v18_algebra_multistep_64_actionz",
        "seed": int(seed),
        "n_states": int(n_states),
        "n_views": int(n_views),
        "y_classes": int(y_classes),
