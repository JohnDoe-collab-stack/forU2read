...
def main() -> None:
    ...
    sources = [
        ...
        (HERE / "aslmt_model_v18_algebra_multistep_64_actionz.py"),
        (HERE / "aslmt_train_v18_algebra_multistep_64_matrix_diagstop_actionz.py"),
        (HERE / "verify_aslmt_v18_algebra_multistep_64_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_v18_algebra_multistep_64_actionz_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    master_jsonl = run_dir / f"v18_algebra_multistep_64_actionz_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n_states in n_list:
        for seed in seeds:
            train = snap_root / "aslmt_train_v18_algebra_multistep_64_matrix_diagstop_actionz.py"
