...
def _partial_verify_ref_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_v18_algebra_multistep_64_matrix.py"
    verify_cmd = [
...
def main() -> None:
    ...
    sources = [
        (HERE / "aslmt_env_v18_algebra_multistep_64.py"),
        (HERE / "aslmt_model_v18_algebra_multistep_64.py"),
        (HERE / "aslmt_model_v18_algebra_multistep_64_stz.py"),
        (HERE / "aslmt_train_v18_algebra_multistep_64_matrix_diagstop_rollout.py"),
        (HERE / "verify_aslmt_v18_algebra_multistep_64_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_v18_algebra_multistep_64_rolloutdiagstop_stz_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v18_algebra_multistep_64_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"v18_algebra_multistep_64_rolloutdiagstop_stz_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n_states in n_list:
        for seed in seeds:
            train = snap_root / "aslmt_train_v18_algebra_multistep_64_matrix_diagstop_rollout.py"
