...
def main() -> None:
    ...
    sources = [
        ...
        (HERE / "aslmt_train_v18_algebra_multistep_64_matrix_diagstop.py"),
        (HERE / "verify_aslmt_v18_algebra_multistep_64_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_v18_algebra_multistep_64_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    master_jsonl = run_dir / f"v18_algebra_multistep_64_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n_states in n_list:
        for seed in seeds:
            train = snap_root / "aslmt_train_v18_algebra_multistep_64_matrix_diagstop.py"
