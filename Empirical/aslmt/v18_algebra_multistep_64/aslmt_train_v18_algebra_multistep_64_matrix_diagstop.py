...
def _eval_split(
    *,
    modelA: V18AlgebraMultistepModelA,
    ...
    # swap effect metrics (query policy)
    swap_action_follow = []
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            # Swap z across batch as a single permutation (reverse), for a strict intervention.
            ...
            # action follow: compare swapped actions with permuted original actions
            a0 = outA["actions"][:, 0]
...
def main() -> None:
    p = argparse.ArgumentParser(description="ASLMT v18 algebra multistep matrix trainer (diagstop).")
    ...
    modelA = V18AlgebraMultistepModelA(cfgA).to(device)
    ...
    # Training stream: mix IID and OOD episodes.
    ...
    for step in range(1, steps + 1):
        ...
        opt_actions = batch["opt_actions"].to(device)  # (B,T)
        opt_responses = batch["opt_responses"].to(device)  # (B,T)

        B = tables.shape[0]
        # teacher forcing: compute query loss per step from current transcript prefix
        ...
        for t in range(q_steps):
            logits_q = modelA.logits_query(tables=tables, sigma=sigma, base_obs=base_obs, actions=actions_prefix, responses=responses_prefix, t=t).clone()
            # forbid base view + repeats during training
            ...
            q_loss_terms.append(F.cross_entropy(logits_q, opt_actions[:, t].to(torch.long)))
            actions_prefix[:, t] = opt_actions[:, t]
            responses_prefix[:, t] = opt_responses[:, t]

        # y loss at end of teacher transcript
        y_logits = modelA.predict_y(tables=tables, sigma=sigma, base_obs=base_obs, actions=opt_actions, responses=opt_responses, t=q_steps)
        y_loss = F.cross_entropy(y_logits, y.to(torch.long))

        # baselines (trained to fail gracefully)
        ...
        if step % (250 if profile == "solid" else 50) == 0 or step == steps:
            with torch.no_grad():
                q_acc = _acc_from_logits(logits_q, opt_actions[:, -1])
            ...
            print(
                ...
                f"y_acc={y_acc:.4f} q_acc={q_acc:.4f}"
            )

    # Eval: strict metrics for master JSONL row
    ...
    rec = {
        "kind": "aslmt_v18_algebra_multistep_64",
        "seed": int(seed),
        "n_states": int(n_states),
        "n_views": int(n_views),
        "y_classes": int(y_classes),
