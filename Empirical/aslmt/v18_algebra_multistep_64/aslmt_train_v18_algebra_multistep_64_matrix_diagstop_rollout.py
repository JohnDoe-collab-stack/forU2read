...
from aslmt_env_v18_algebra_multistep_64 import V18AlgebraMultistepDataset64
from aslmt_model_v18_algebra_multistep_64_stz import (
    AlgebraModelCfg,
    V18AlgebraCueOnlyBaseline,
    V18AlgebraMultistepModelA_STZ,
...
def _candidate_mask_from_transcript(
    *,
    tables_vn: torch.Tensor,  # (V,N)
    base_obs: int,
    actions: list[int],
    responses: list[int],
) -> torch.Tensor:
    """
    Boolean mask over x ∈ X consistent with the transcript.
    tables_vn lives on device.
    """
    m = tables_vn[0].eq(int(base_obs))
    for a, r in zip(actions, responses):
        m = m & tables_vn[int(a)].eq(int(r))
    return m


def _sigma_ambiguity(*, sigma_n: torch.Tensor, cand_mask: torch.Tensor) -> int:
    vals = sigma_n[cand_mask]
    if int(vals.numel()) == 0:
        return 0
    return int(torch.unique(vals).numel())


def _greedy_next_action(
    *,
    tables_vn: torch.Tensor,  # (V,N)
    sigma_n: torch.Tensor,  # (N,)
    x: int,
    base_obs: int,
    chosen_actions: list[int],
    chosen_responses: list[int],
    forbid_view0: bool = True,
) -> int:
    """
    Greedy oracle conditioned on the *current transcript* (not necessarily the dataset's
    original opt_actions prefix).
    """
    V = int(tables_vn.shape[0])
    best_a: int | None = None
    best_score = None
    used = set(int(a) for a in chosen_actions)
    ...
    for a in range(V):
        ...
        if a in used:
            continue
        r = int(tables_vn[a, int(x)].item())
        cand = _candidate_mask_from_transcript(
            tables_vn=tables_vn,
            base_obs=int(base_obs),
            actions=chosen_actions + [int(a)],
            responses=chosen_responses + [int(r)],
        )
        amb = _sigma_ambiguity(sigma_n=sigma_n, cand_mask=cand)
    ...
    if best_a is None:
        # Should not happen for v>=3, but keep shapes stable.
...
def _eval_split(
    *,
    modelA: V18AlgebraMultistepModelA_STZ,
    ...
    ds = V18AlgebraMultistepDataset64(
        size=int(cfg.n_eval),
    ...
    with torch.no_grad():
        for batch in dl:
            tables = batch["tables"].to(device)  # (B,V,N)
            sigma = batch["sigma"].to(device)  # (B,N)
            x = batch["x"].to(device)  # (B,)
            y = batch["y"].to(device)  # (B,)
            base_obs = batch["base_obs"].to(device)  # (B,)
...
def main() -> None:
    p = argparse.ArgumentParser(description="ASLMT v18 algebra multistep matrix trainer (rollout-aligned diagstop).")
    ...
    p.add_argument("--z-classes", type=int, default=16)
    ...
    modelA = V18AlgebraMultistepModelA_STZ(cfgA).to(device)
    ...
    ds_iid = V18AlgebraMultistepDataset64(
    ...
    ds_ood = V18AlgebraMultistepDataset64(
        size=max(1024, steps * batch_size),
        n_states=n_states,
        n_views=n_views,
        y_classes=y_classes,
    ...
    for step in range(1, steps + 1):
        ...
        # scheduled teacher forcing probability
        ...
        for t in range(q_steps):
            ...
            # Dynamic oracle conditioned on the current transcript.
            with torch.no_grad():
                tgt = torch.zeros((B,), device=device, dtype=torch.long)
                for b in range(B):
                    chosen_actions = [int(a.item()) for a in actions_prefix[b, : int(t)]]
                    chosen_responses = [int(r.item()) for r in responses_prefix[b, : int(t)]]
                    a_star = _greedy_next_action(
                        tables_vn=tables[b],
                        sigma_n=sigma[b],
                        x=int(x[b].item()),
            ...
            q_loss_terms.append(F.cross_entropy(logits_q, tgt))

            with torch.no_grad():
                pred_a = torch.argmax(logits_q, dim=-1).to(torch.long)
                rollout_q_acc_terms.append(float((pred_a == tgt).float().mean().item()))

                # choose next action: teacher vs model
                use_teacher = (torch.rand((B,), device=device) < float(p_tf)).to(torch.bool)
                a_next = torch.where(use_teacher, tgt, pred_a).to(torch.long)
                actions_prefix[:, t] = a_next

                rr = tables[torch.arange(B, device=device), a_next, x.to(torch.long)]
                responses_prefix[:, t] = rr.to(torch.long)

        # y loss on the *actual* transcript we built (rollout-aligned)
    ...
    rec = {
        "kind": "aslmt_v18_algebra_multistep_64_rolloutdiagstop_stz",
        "seed": int(seed),
        "n_states": int(n_states),
        "n_views": int(n_views),
        "y_classes": int(y_classes),
