...
def main() -> None:
    ...
    # v2: match the v15-style stability knobs by default.
    p.add_argument("--w-dice", type=float, default=0.25)
    p.add_argument("--bce-pos-weight", type=str, default="batch")
    args = p.parse_args()
    ...
    tag = f"aslmt_law_v3b_unified_v2_strong_qforced_zread_stability_n_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_law_v3b_unified_v2_strong_qforced_zread_stability_n_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"law_v3b_unified_v2_strong_qforced_zread_stability_n_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n in n_list:
        ...
        for z in zs:
            for seed in seeds:
                ...
                if int(r.returncode) != 0:
                    raise SystemExit(int(r.returncode))
