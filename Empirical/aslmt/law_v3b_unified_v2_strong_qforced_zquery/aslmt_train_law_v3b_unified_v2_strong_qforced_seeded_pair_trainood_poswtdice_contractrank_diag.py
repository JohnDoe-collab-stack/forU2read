...
from aslmt_model_law_v3b_unified_v2_strong_qforced_query import (
...
def _env_res_bit(*, h: torch.Tensor, k: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
    ...
    Critical change vs the flip-based response:
...
def main() -> None:
    ...
    out = {
        "kind": "aslmt_law_v3b_unified_v2_strong_qforced_zquery_poswtdice_contractrank_spaced2_64",
        "seed": int(cfg.seed),
        "n_classes": int(n_classes),
    ...
    if str(args.out_faildump_jsonl).strip():
        ...
        if need_dump:
            ...
            rec = {
                "kind": "aslmt_law_v3b_unified_v2_strong_qforced_zquery_faildump_image_gate",
                "seed": int(cfg.seed),
                "n_classes": int(n_classes),
                "z_classes": int(z_classes),
