...
def main() -> None:
    ...
    sources = [
        (HERE / "aslmt_model_law_v3b_unified_v2_strong_qforced_query.py"),
        ...
        (HERE / "aslmt_train_law_v3b_unified_v2_strong_qforced_seeded_pair_trainood_poswtdice_contractrank_diag.py"),
        (HERE / "verify_aslmt_law_v3b_unified_v2_strong_qforced_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_law_v3b_unified_v2_strong_qforced_zquery_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_law_v3b_unified_v2_strong_qforced_zquery_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"law_v3b_unified_v2_strong_qforced_zquery_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n in n_list:
        ...
        for z in zs:
            for seed in seeds:
                train = snap_root / "aslmt_train_law_v3b_unified_v2_strong_qforced_seeded_pair_trainood_poswtdice_contractrank_diag.py"
