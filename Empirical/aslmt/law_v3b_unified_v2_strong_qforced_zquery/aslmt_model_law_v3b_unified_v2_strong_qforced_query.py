...
class LawV3bUnifiedV2StrongQForcedQueryModelA(nn.Module):
    """
    v3b-style unified strong variant:

    - `z` carries the missing hidden information (`h`) from cue only.
    - `k` is not visible in `image`. It must be obtained via a query response channel.
    - The decoder is explicitly conditioned on both `z` and a query-provided response bit.
    - The query head is a function of `z` (not a separate marker-to-query head). This is the
      "zquery" variant: the action is derived from the internal mediator `z`.
    """
    ...
    def __init__(self, *, z_classes: int, hid: int = 32):
        ...
        # Condition decoder on: (image, z one-hot, response bit one-hot)
        self.dec = _UNet32(3 + self.z_classes + 2, base=int(hid))

        # Query head derived from z logits only (zquery design).
        # We keep this as a simple linear map so that, once z is correct, query can become correct
        # without needing to relearn a parallel cue->query pipeline.
        self.query_from_z = nn.Linear(self.z_classes, 2)

    def logits_query(self, cue_image: torch.Tensor) -> torch.Tensor:
        # Important: query depends only on z_logits.
        # We detach z_logits so the query supervision does not backprop into z_mlp. This keeps the
        # "z carries h" contract clean: z is trained by the z loss, query is trained by learning
        # how to read z.
        z_logits = self.z_logits(cue_image).detach()
        return self.query_from_z(z_logits)
