...
def main() -> None:
    ...
    sources = [
        HERE / "render_v12_clean64_paired_ctx.py",
        HERE / "aslmt_env_v12_clean64.py",
        HERE / "aslmt_model_v12_clean64_kdet.py",
        HERE / "aslmt_train_v12_clean64.py",
        HERE / "verify_aslmt_v12_phaseA2_clean64_matrix.py",
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)

    snap_root = SNAP_DIR / f"aslmt_v12_clean64_phaseA2_{ts}_{bundle_hash[:12]}"
    snap_root.mkdir(parents=True, exist_ok=False)
    for pth in sources:
        shutil.copy2(pth, snap_root / pth.name)

    manifest = {
        "kind": "aslmt_v12_clean64_phaseA2_snapshot",
        "timestamp": ts,
    ...
    run_dir = RUNS_DIR / f"aslmt_v12_clean64_phaseA2_{ts}_{bundle_hash[:12]}"
    run_dir.mkdir(parents=True, exist_ok=False)
    master_jsonl = run_dir / f"v12_clean64_master_{ts}_{bundle_hash[:12]}.jsonl"

    n_list = _parse_int_list(args.n_classes_list)
    seeds = list(range(int(args.seed_from), int(args.seed_to) + 1))

    train = snap_root / "aslmt_train_v12_clean64.py"
    for n in n_list:
        for z in _zs_for_n(int(n)):
            for seed in seeds:
    ...
    verify = snap_root / "verify_aslmt_v12_phaseA2_clean64_matrix.py"
