...
from aslmt_env_v12_clean64 import V12Clean64Dataset
from aslmt_model_v12_clean64_kdet import V12Clean64CueOnlyBaseline, V12Clean64ImageOnlyBaseline, V12Clean64ModelA
from render_v12_clean64_paired_ctx import Ctx, POS_STRIDE, render, min_occ_half_for_n
...
def _center_of_mass(mask01: torch.Tensor) -> torch.Tensor:
    ...
    xy = _xy_grid(H=H, W=W, device=mask01.device, dtype=mask01.dtype)
    w = mask01
    mass = w.sum(dim=(1, 2, 3), keepdim=True).clamp_min(1e-6)
    cx = (w * xy[:, 0:1]).sum(dim=(1, 2, 3), keepdim=True) / mass
    cy = (w * xy[:, 1:2]).sum(dim=(1, 2, 3), keepdim=True) / mass
    return torch.cat([cx, cy], dim=1).squeeze(-1).squeeze(-1)
...
def _sample_ctxs(*, n: int, seed: int, ood: bool, img_size: int, n_classes: int) -> list[Ctx]:
    ...
    inner = 2
    occ_min = int(min_occ_half_for_n(n_classes=int(n_classes), inner=inner, t_max=4))
    margin = 4
    max_occ = (int(img_size) // 2) - margin
    if occ_min > max_occ:
        raise ValueError(f"img_size={img_size} too small for n_classes={n_classes} (occ_min={occ_min} > max_occ={max_occ})")
    hi_occ = min(max_occ + 1, occ_min + (3 if bool(ood) else 2))
    occ_half = torch.randint(low=int(occ_min), high=int(hi_occ), size=(n,), generator=g).tolist()
...
def _img_pair_rank_loss(*, modelA: V12Clean64ModelA, device: torch.device, n_classes: int, n_ctx: int, seed: int, ood: bool, img_size: int) -> torch.Tensor:
    ...
    for ctx in ctxs:
        ...
        x0 = render(ctx=ctx, h=h0, k=k_fixed, n_classes=int(n_classes))
        x1 = render(ctx=ctx, h=h1, k=k_fixed, n_classes=int(n_classes))
...
def _cue_pair_rank_loss(*, modelA: V12Clean64ModelA, device: torch.device, n_classes: int, n_ctx: int, seed: int, ood: bool, img_size: int) -> torch.Tensor:
    ctxs = _sample_ctxs(n=int(n_ctx), seed=int(seed), ood=bool(ood), img_size=int(img_size), n_classes=int(n_classes))
    g = torch.Generator()
    ...
    for ctx in ctxs:
        ...
        x0 = render(ctx=ctx, h=h_fixed, k=0, n_classes=int(n_classes))
        x1 = render(ctx=ctx, h=h_fixed, k=1, n_classes=int(n_classes))
...
def _pair_eval_one(
    *,
    modelA: V12Clean64ModelA,
    modelB_img: V12Clean64ImageOnlyBaseline,
    modelB_cue: V12Clean64CueOnlyBaseline,
    device: torch.device,
    n_ctx: int,
    seed: int,
    ood: bool,
    ...
    for i, ctx in enumerate(ctxs):
        ...
        x_im0 = render(ctx=ctx, h=h0, k=k_fixed, n_classes=int(n_classes))
        x_im1 = render(ctx=ctx, h=h1, k=k_fixed, n_classes=int(n_classes))
        ...
        x_cu0 = render(ctx=ctx, h=h_fixed, k=0, n_classes=int(n_classes))
        x_cu1 = render(ctx=ctx, h=h_fixed, k=1, n_classes=int(n_classes))
        ...
        if bool(_overlap_score(pA_im0, t_im0) > _overlap_score(pA_im0, t_im1)) and bool(_overlap_score(pA_im1, t_im1) > _overlap_score(pA_im1, t_im0)):
            A_both_image += 1
        if bool(_overlap_score(pA_cu0, t_cu0) > _overlap_score(pA_cu0, t_cu1)) and bool(_overlap_score(pA_cu1, t_cu1) > _overlap_score(pA_cu1, t_cu0)):
            A_both_cue += 1
        if bool(_overlap_score(pB_img, t_im0) > _overlap_score(pB_img, t_im1)) and bool(_overlap_score(pB_img, t_im1) > _overlap_score(pB_img, t_im0)):
            B_img_both += 1
        if bool(_overlap_score(pB_cue, t_cu0) > _overlap_score(pB_cue, t_cu1)) and bool(_overlap_score(pB_cue, t_cu1) > _overlap_score(pB_cue, t_cu0)):
            B_cue_both += 1
        if bool(_overlap_score(pA_im0_abl, t_im0) > _overlap_score(pA_im0_abl, t_im1)) and bool(_overlap_score(pA_im1_abl, t_im1) > _overlap_score(pA_im1_abl, t_im0)):
            A_abl_both_image += 1
        if bool(_overlap_score(pA_im0_swap, t_im1) > _overlap_score(pA_im0_swap, t_im0)) and bool(_overlap_score(pA_im1_swap, t_im0) > _overlap_score(pA_im1_swap, t_im1)):
            A_swap_follow_image += 1
        if bool(_overlap_score(pA_im0_swap, t_im0) > _overlap_score(pA_im0_swap, t_im1)) and bool(_overlap_score(pA_im1_swap, t_im1) > _overlap_score(pA_im1_swap, t_im0)):
...
def main() -> None:
    ...
    modelA = V12Clean64ModelA(z_classes=int(z_classes)).to(device)
    modelB_img = V12Clean64ImageOnlyBaseline().to(device)
    modelB_cue = V12Clean64CueOnlyBaseline().to(device)
    ...
    ds_iid = V12Clean64Dataset(size=50_000, n_classes=int(n_classes), ood=False, img_size=int(cfg.img_size), seed=int(cfg.seed))
    ds_ood = V12Clean64Dataset(size=50_000, n_classes=int(n_classes), ood=True, img_size=int(cfg.img_size), seed=int(cfg.seed) + 1_000_000)
    ...
    def _eval_split(*, ood: bool) -> dict:
        ds = V12Clean64Dataset(size=1024, n_classes=int(n_classes), ood=bool(ood), img_size=int(cfg.img_size), seed=int(cfg.seed) + (2_000_000 if ood else 0))
    ...
    pair_eval = {
        "iid": _pair_eval_one(modelA=modelA, modelB_img=modelB_img, modelB_cue=modelB_cue, device=device, n_ctx=int(cfg.pair_n_ctx), seed=int(cfg.seed), ood=False, img_size=int(cfg.img_size), n_classes=int(n_classes)),
        "ood": _pair_eval_one(modelA=modelA, modelB_img=modelB_img, modelB_cue=modelB_cue, device=device, n_ctx=int(cfg.pair_n_ctx), seed=int(cfg.seed), ood=True, img_size=int(cfg.img_size), n_classes=int(n_classes)),
    }

    out = {
        "kind": "aslmt_v12_clean64",
        "seed": int(cfg.seed),
        "n_classes": int(n_classes),
