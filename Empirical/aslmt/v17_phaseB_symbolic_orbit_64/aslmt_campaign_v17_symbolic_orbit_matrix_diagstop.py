...
def _partial_verify_ref_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_v17_symbolic_orbit_matrix.py"
    verify_cmd = [
...
def main() -> None:
    ...
    sources = [
        ...
        (HERE / "aslmt_env_v17_symbolic_orbit_nscalable_64.py"),
        (HERE / "render_v17_symbolic_orbit_paired_ctx_64.py"),
        (HERE / "aslmt_train_v17_symbolic_orbit_seeded_pair_trainood_poswtdice_contractrank_diag.py"),
        (HERE / "verify_aslmt_v17_symbolic_orbit_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)

    tag = f"aslmt_v17_phaseB_symbolic_orbit_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v17_phaseB_symbolic_orbit_snapshot",
        "timestamp": ts,
    ...
    master_jsonl = run_dir / f"v17_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n in n_list:
        ...
        for z in zs:
            for seed in seeds:
                train = snap_root / "aslmt_train_v17_symbolic_orbit_seeded_pair_trainood_poswtdice_contractrank_diag.py"
