...
def main() -> None:
    p = argparse.ArgumentParser(description="Strict verifier for ASLMT v17 Phase B (symbolic orbit) matrix runs.")
            ...
            if int(z) == int(n):
                for sp in ("iid", "ood"):
                    ...
                    if solid:
                        ...
                        # Smoke profile is wiring-only: require barrier checks + presence of numeric metrics.
                        # Performance thresholds are enforced only under profile=solid.
                        if not (math.isfinite(A_img_min) and math.isfinite(A_cue_min)):
                            failures.append(f"ref n={n} z={z} {sp}: missing A pair rates (A_img_min={A_img_min}, A_cue_min={A_cue_min})")
                        if not (math.isfinite(Bimg_max) and math.isfinite(Bcue_max)):
                            failures.append(f"ref n={n} z={z} {sp}: missing baseline rates (Bimg_max={Bimg_max}, Bcue_max={Bcue_max})")
                        if not (math.isfinite(swap_follow_min) and math.isfinite(swap_orig_max)):
                            failures.append(f"ref n={n} z={z} {sp}: missing swap rates (swap_follow_min={swap_follow_min}, swap_orig_max={swap_orig_max})")
                        if not math.isfinite(abl_img_max):
                            failures.append(f"ref n={n} z={z} {sp}: missing ablation rate (abl_img_max={abl_img_max})")
            else:
                for sp in ("iid", "ood"):
                    if not _all_barriers_ok(sp):
    ...
    if failures:
        print("\n[FAIL] v17 Phase B (symbolic orbit) matrix checks did not pass:")
        for s in failures:
            print("-", s)
        raise SystemExit(1)

    print("\n[OK] v17 Phase B (symbolic orbit) matrix checks passed.")
