...
def sample_ctx(*, idx: int, n_classes: int, ood: bool, img_size: int, seed_base: int) -> Ctx:
    ...
    if bool(ood):
        horizon = int(torch.randint(low=12, high=21, size=(1,), generator=g).item())
    else:
        horizon = int(torch.randint(low=6, high=12, size=(1,), generator=g).item())
...
def render(ctx: Ctx, *, h: int, k: int, n_classes: int) -> dict[str, torch.Tensor]:
    """
    Symbolic family (orbit mask):

    - `h` is visible only in `cue_image` (start marker).
    - `k` is visible only in `image` (present stripe).
    - The hidden target is an orbit of points under an affine map whose seed is `h`,
      with parameters coupled to `k` and `ctx.horizon`.
    """
    n = int(n_classes)
    assert n >= 2
    ...
    _draw_point(cue, start_x + 1, start_y, H, 1.0)

    if kk == 1:
        image[0:3, :] = 1.0

    # Background dots depend on ctx only, never on h.
    g = torch.Generator().manual_seed(int(ctx.seed) + (101 if kk == 1 else 17))
    n_dots = int(torch.randint(low=0, high=(9 if ctx.ood else 5), size=(1,), generator=g).item())
    ...
    # Orbit parameters (affine steps); k changes the orbit.
    a = 3 + kk
    b = 5 + 2 * kk
    c = 7 + kk
    d = 11 + 3 * kk
    ...
    for i in range(int(ctx.horizon)):
        ...
        x = int(margin + ((x - margin) + a * i + b) % (W - 2 * margin))
        y = int(margin + ((y - margin) + c * i + d) % (H - 2 * margin))
...
def main() -> None:
    p = argparse.ArgumentParser(description="Render paired-ctx witnesses for ASLMT v17 symbolic orbit family (64x64).")
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else (runs_dir / f"aslmt_v17_symbolic_orbit_paired_ctx_{suf}")
    ...
    manifest = {
        "kind": "aslmt_v17_symbolic_orbit_paired_ctx",
        "timestamp": ts,
