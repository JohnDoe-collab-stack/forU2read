...
def main() -> None:
    p = argparse.ArgumentParser(
        description="v20 algebra v3b proofpack v2: train → certify counterexamples → verify (IID+OOD) + minproof (finite mediator collision witnesses)."
    )
    ...
    run_dir = RUNS_DIR / f"aslmt_v20_algebra_v3b_proofpack_v3_{ts}_{bundle_short}"
    run_dir.mkdir(parents=True, exist_ok=False)
    snap_root = SNAP_DIR / f"aslmt_v20_algebra_v3b_proofpack_v3_{ts}_{bundle_short}"
    ...
    z_list = sorted(set(list(range(1, int(z_max) + 1)) + z_list_in))
        ...
        for z in z_list:
            for seed in seeds:
                ...
                for split in ["iid", "ood"]:
                    ...
                    _run_with_timestamped_log(cmd=cert_marg_cmd, out=cert_marg_log)
                    ...
                    _run_with_timestamped_log(cmd=verify_marg_cmd, out=verify_marg_log)
                    ...
                    _run_with_timestamped_log(cmd=cert_min_cmd, out=cert_min_log)
                    ...
                    _run_with_timestamped_log(cmd=verify_min_cmd, out=verify_min_log)
