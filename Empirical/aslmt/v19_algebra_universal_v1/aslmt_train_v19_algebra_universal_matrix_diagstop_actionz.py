...
from aslmt_env_v19_algebra_universal import V19AlgebraUniversalDataset
from aslmt_model_v19_algebra_universal_actionz import (
...
def _allowed_actions_one_step(
    *,
    ...
    forbid_view0: bool,
    allow_stop_if_closed: bool,
) -> list[int]:
    """
    Oracle v1 (myopic): allowed actions are those minimizing expected ambiguity after one step.

    Additionally, STOP (action==V) is allowed iff the current transcript is already closed.
    """
    V, _ = tables.shape
    cand0 = _candidate_mask(
        tables=tables, base_obs=int(base_obs), actions=list(actions_prefix), responses=list(responses_prefix)
    )
    amb0 = _sigma_ambiguity(sigma=sigma, cand_mask=cand0)
    if bool(allow_stop_if_closed) and amb0 == 1:
        return [int(V)]  # STOP

    idx0 = cand0.nonzero(as_tuple=False).flatten()
    if int(idx0.numel()) == 0:
        return [int(V)]  # degenerate: stop
    ...
    for a in range(int(V)):
        ...
        best = min(best, float(ea))
        scores.append((int(a), float(ea)))

    allowed = [a for (a, s) in scores if float(s) == float(best)]
    if not allowed:
        # fallback: if everything forbidden/used, stop
        return [int(V)]
    return sorted({int(a) for a in allowed})


def _multi_positive_ce(
    *,
    logits: torch.Tensor,  # (B,A)
    allowed: torch.Tensor,  # (B,A) bool
) -> torch.Tensor:
    """
    Multi-positive cross entropy:
      loss = -log sum_{a in allowed} softmax(logits)[a]
    """
...
def main() -> None:
    p = argparse.ArgumentParser(description="ASLMT v19 algebra universal v1 trainer (action-as-z diagstop).")
    ...
    for step in range(1, int(steps) + 1):
        ...
        y = batch["y"].to(device)
        base_obs = batch["base_obs"].to(device)
        opt_actions = batch["opt_actions"].to(device)  # (B, <=max_steps+1), padded with STOP
        ...
        for t in range(int(max_steps)):
            ...
            # Allowed-set target computed from oracle (independent of the model).
            ...
            for b in range(int(B)):
                ...
                allow = _allowed_actions_one_step(
            ...
            # Teacher action (sample uniformly from allowed set).
            # This avoids arbitrary tie-breaking and keeps supervision constructive.
            teacher = torch.zeros((B,), device=device, dtype=torch.long)
            for b in range(int(B)):
                idxs = torch.nonzero(allowed[b], as_tuple=False).flatten()
                if int(idxs.numel()) == 0:
                    teacher[b] = int(V)  # STOP
                else:
        ...
        y_logits = modelA.predict_y(
            tables=tables, sigma=sigma, base_obs=base_obs, actions=actions_prefix, responses=responses_prefix, t=int(max_steps)
        )
    ...
    rec = {
        "kind": "aslmt_v19_algebra_universal_actionz_v1",
        "seed": int(args.seed),
        "n_states": int(n_states),
        "n_views": int(n_views),
        "y_classes": int(y_classes),
