...
def _partial_verify_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_v19_algebra_universal_matrix.py"
    verify_cmd = [
...
def main() -> None:
    ...
    sources = [
        (HERE / "aslmt_env_v19_algebra_universal.py"),
        (HERE / "aslmt_model_v19_algebra_universal_actionz.py"),
        (HERE / "aslmt_train_v19_algebra_universal_matrix_diagstop_actionz.py"),
        (HERE / "verify_aslmt_v19_algebra_universal_matrix.py"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_v19_algebra_universal_actionz_v1_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v19_algebra_universal_actionz_v1_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"v19_algebra_universal_actionz_v1_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n_states in n_list:
        for seed in seeds:
            train = snap_root / "aslmt_train_v19_algebra_universal_matrix_diagstop_actionz.py"
