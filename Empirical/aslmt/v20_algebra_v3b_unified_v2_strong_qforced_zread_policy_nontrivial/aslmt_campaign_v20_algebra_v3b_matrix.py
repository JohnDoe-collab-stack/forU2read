...
def _partial_verify_ref_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_v20_algebra_v3b_matrix.py"
    verify_cmd = [
        str(py),
        "-u",
        str(verify),
...
def main() -> None:
    ...
    sources = [
        ...
        (HERE / "aslmt_campaign_v20_algebra_v3b_matrix.py"),
    ...
    tag = f"aslmt_v20_algebra_v3b_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v20_algebra_v3b_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"v20_algebra_v3b_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n in n_list:
        if str(args.z_policy) == "explicit":
            zs = _parse_int_list(args.z_classes_list)
        else:
            zs = _zs_ref_first(int(n))
        if str(profile) == "smoke" and int(n) not in zs:
            zs = [int(n)]
        for z in zs:
            for seed in seeds:
    ...
    if str(args.z_policy) == "explicit":
        verify_cmd += ["--z-classes-list", str(args.z_classes_list)]
