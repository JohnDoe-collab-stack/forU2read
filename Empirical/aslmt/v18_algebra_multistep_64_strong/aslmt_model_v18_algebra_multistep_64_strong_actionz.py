...
class V18AlgebraMultistepModelA_ActionZ_Strong(nn.Module):
    """
    Strong query policy model:

    The query logits depend on a 2-step closure lookahead computed from (tables, σ, transcript).

    At time t, each candidate view j is scored by the expected final σ-ambiguity after:
      - taking j now,
      - then taking the best remaining view after observing the response.
