# ASLMT v19 — Algebra Universal v5.1 (learned encoder policy, strict protocol)

v5.1 builds on v5 and keeps the same compatibility-aware belief update, but makes the “stable eval”
logging actually stable by forcing `model.eval()` (disables dropout), and logs mean±std.
...
New in v4.9:
- modify the model belief `b(s)` to include a differentiable compatibility score between each transcript
  step `(a_t, r_t)` and the state-specific view label `tables[a_t, s]` (soft analog of candidate masking).

New in v5:
- add a stable periodic eval log during training (IID+OOD), controlled by:
  `--log-eval-every`, `--log-eval-n`, `--log-eval-batch-size`.

New in v5.1:
- stable eval uses `model.eval()` (disables dropout), then restores original mode;
- stable eval prints mean±std for `A_acc` and `closed_rate_roll`;
- the master JSONL metrics include `*_std` fields for split metrics.

This folder is a new protocol variant (v5.1). It does **not** modify v1/v2/v3/v4.1/v4.3/v4.4/v4.5/v4.6/v4.7/v4.8/v4.9/v5 scripts or historical run snapshots.
...
v19_algebra_universal_v5_1/aslmt_campaign_v19_algebra_universal_v5_1_matrix_diagstop_actionz.py \
...
run_dir="$(ls -1dt aslmt_v19_algebra_universal_actionz_v5_1_solid_* | head -n 1)"
...
```bash
/home/frederick/.venvs/cofrs-gpu/bin/python3 -u \
/mnt/c/Users/frederick/Documents/forU2read/Empirical/aslmt/v19_algebra_universal_v5_1/audit_v19_algebra_universal_v5_1_algebra.py \
