...
def _partial_verify_or_die(
    *,
    ...
    verify = snap_root / "verify_aslmt_v19_algebra_universal_v5_1_matrix.py"
    verify_cmd = [
        str(py),
        "-u",
        str(verify),
...
def main() -> None:
    ...
    p.add_argument("--log-eval-every", type=int, default=1000)
    p.add_argument("--log-eval-n", type=int, default=512)
    p.add_argument("--log-eval-batch-size", type=int, default=64)
    args = p.parse_args()
    ...
    sources = [
        ...
        (HERE / "verify_aslmt_v19_algebra_universal_v5_1_matrix.py"),
        # Protocol provenance (document + orchestrator + independent audit).
        (HERE / "aslmt_campaign_v19_algebra_universal_v5_1_matrix_diagstop_actionz.py"),
        (HERE / "audit_v19_algebra_universal_v5_1_algebra.py"),
        (HERE / "README_aslmt_v19_algebra_universal_v5_1.md"),
    ]

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    bundle_hash = _bundle_hash(sources)
    tag = f"aslmt_v19_algebra_universal_actionz_v5_1_{profile}_{ts}_{bundle_hash[:12]}"
    ...
    manifest = {
        "kind": "aslmt_v19_algebra_universal_actionz_v5_1_snapshot",
        "timestamp": ts,
        "profile": str(profile),
    ...
    master_jsonl = run_dir / f"v19_algebra_universal_actionz_v5_1_{profile}_master_{ts}_{bundle_hash[:12]}.jsonl"
    ...
    for n_states in n_list:
        for seed in seeds:
            ...
            cmd = [
                ...
                "--log-eval-every",
                str(int(args.log_eval_every)),
                "--log-eval-n",
                str(int(args.log_eval_n)),
                "--log-eval-batch-size",
                str(int(args.log_eval_batch_size)),
                "--master-jsonl",
                str(master_jsonl),
            ]
