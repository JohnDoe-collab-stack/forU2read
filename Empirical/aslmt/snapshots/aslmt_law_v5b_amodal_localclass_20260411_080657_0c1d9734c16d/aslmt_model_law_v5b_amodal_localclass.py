...
class LawV5bVisibleOnlyLocalFCNDeep(nn.Module):
    """
    Local baseline B2 (deeper):
    - still strictly local (no pooling, no global ops)
    - larger receptive field than B1 but still bounded

    Receptive field size for L conv layers (k=3) is 1 + 2L.
    Here L=8 => RF=17 (still < 32).
    """

    def __init__(self, *, width: int = 128, n_layers: int = 8):
        super().__init__()
        w = int(width)
        L = int(n_layers)
        if L < 2:
            raise ValueError("n_layers must be >= 2")
        layers: list[nn.Module] = [nn.Conv2d(1, w, kernel_size=3, padding=1), nn.GELU()]
        for _ in range(L - 2):
            layers += [nn.Conv2d(w, w, kernel_size=3, padding=1), nn.GELU()]
        layers += [nn.Conv2d(w, 1, kernel_size=3, padding=1)]
        self.net = nn.Sequential(*layers)
        self.receptive_field = 1 + 2 * L
...
def local_baseline_builders(*, img_size: int = 32) -> dict[str, nn.Module]:
    return {
        ...
        "B2_local_fcn_deep": LawV5bVisibleOnlyLocalFCNDeep(width=128, n_layers=8),
