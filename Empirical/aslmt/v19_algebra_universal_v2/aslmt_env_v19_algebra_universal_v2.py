...
from aslmt_oracle_v19_algebra_universal_v2_common import (
    HorizonOracleCfg,
    candidate_mask,
...
def sample_episode(
    *,
    ...
    Families (same as v1; v2 fixes the oracle/supervision to be horizon-consistent):
    ...
    if y != 2:
        raise ValueError("v19 universal v2 currently requires y_classes=2")
    if base_view != 0:
        raise ValueError("v19 universal v2 expects base_view=0")
    if v < 3:
    ...
    # Barrier: base-only must never be closed. In this construction it is enough to check
    # that within a base bucket, σ attains both values {0,1}. (Then Amb_σ(F_base)=2.)
    for _ in range(3):
        b0 = int(_randint(g, 0, int(n_base), (1,)).item())
        m0 = base_raw.eq(int(b0))
