...
from aslmt_env_law_v3b_unified_v1_nscalable_spaced2_64 import LawV3bUnifiedV1DatasetNScalableSpaced2_64
from aslmt_model_law_v3b_unified_v1_query import (
    LawV3bUnifiedV1CueOnlyBaseline,
    LawV3bUnifiedV1ImageOnlyBaseline,
    LawV3bUnifiedV1QueryModelA,
)
from render_law_v3b_unified_v1_paired_ctx_nscalable_spaced2_64 import Ctx, POS_STRIDE, render
...
def _img_pair_contract_rank_loss(
    *,
    modelA: LawV3bUnifiedV1QueryModelA,
    device: torch.device,
    n_classes: int,
    ctxs: list[Ctx],
    start_index: int,
    ...
    for j, ctx in enumerate(ctxs):
        ...
        p_pair_swap = modelA.swap_forward(cue_pair, img_pair, perm=perm)
...
def _pair_eval_one(
    *,
    modelA: LawV3bUnifiedV1QueryModelA,
    modelB_img: LawV3bUnifiedV1ImageOnlyBaseline,
    modelB_cue: LawV3bUnifiedV1CueOnlyBaseline,
    device: torch.device,
    cfg: PairEvalCfg,
    ood: bool,
    n_classes: int,
...
def main() -> None:
    ...
    modelB_img = LawV3bUnifiedV1ImageOnlyBaseline().to(device)
    modelB_cue = LawV3bUnifiedV1CueOnlyBaseline().to(device)
    ...
    for step in range(1, int(cfg.steps) + 1):
        ...
        lossA = lossA_seg + float(cfg.w_z) * lossA_z + float(cfg.w_pos) * lossA_pos + float(cfg.w_rank_img) * lossA_rank_img + float(cfg.w_rank_cue) * lossA_rank_cue
    ...
    def _eval_split(*, ood: bool) -> dict:
        ...
        ds = LawV3bUnifiedV1DatasetNScalableSpaced2_64(
    ...
    out = {
        "kind": "aslmt_law_v3b_unified_v1_poswtdice_contractrank_spaced2_64",
        "seed": int(cfg.seed),
        "n_classes": int(n_classes),
    ...
    if str(args.out_faildump_jsonl).strip():
        ...
        if need_dump:
            ...
            rec = {
                "kind": "aslmt_law_v3b_unified_v1_faildump_image_gate",
                "seed": int(cfg.seed),
                "n_classes": int(n_classes),
                "z_classes": int(z_classes),
