...
from pldm.cofrs.config import COFRSConfig
from pldm.cofrs.d4rl_audit import run_cofrs_audit_hierarchical_d4rl
from pldm.cofrs.d4rl_closure import run_cofrs_closure_probe_flat_vs_hierarchical
    ...
    def evaluate(self):
        ...
        if not self.config.disable_l2_planning and self.config.eval_l2:
            ...
            for i, level in enumerate(levels):
                ...
                # Optional small-N probe that compares flat vs hierarchical planning on the
                # same env instances (the probe restores env state afterward).
                if getattr(self.config.cofrs, "enabled", False) and bool(
                    getattr(self.config.cofrs, "closure_probe_enabled", False)
                ):
                    cofrs_probe = run_cofrs_closure_probe_flat_vs_hierarchical(
                        planning_evaluator=planning_evaluator,
                        config=self.config.cofrs,
                    )
                    log_dict.update(cofrs_probe)
                ...
                # COFRS-style behavioral audit on the hierarchical loop (small-N).
                # This logs into `log_dict` and via Logger (jsonl / wandb).
                if getattr(self.config.cofrs, "enabled", False):
                    cofrs_log = run_cofrs_audit_hierarchical_d4rl(
                        planning_evaluator=planning_evaluator,
                        config=self.config.cofrs,
                    )
                    log_dict.update(cofrs_log)
